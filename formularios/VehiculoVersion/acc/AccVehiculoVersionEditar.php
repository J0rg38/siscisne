...
//	Parametro9 = VcsNombre
	...
	$ResVehiculoVersionCaracteristica = $_SESSION['InsVehiculoVersionCaracteristica'.$Identificador]->MtdObtenerSesionObjetos(false);
...
/*	//$InsVehiculoVersion->VehiculoVersionCaracteristica = array();	
	...
	if($InsVehiculoVersion->MtdEditarVehiculoVersion()){					
...
<?php
			...
			$Edito = true;
			$Resultado.='#SAS_VVE_102';
			FncCargarDatos();			
		}else{			
			$Resultado.='#ERR_VVE_102';		
		}			
...
function FncCargarDatos(){
	...
	$_SESSION['SesVveFoto'.$Identificador] = $InsVehiculoVersion->VveFoto;
	$_SESSION['SesVveArchivo'.$Identificador] = $InsVehiculoVersion->VveArchivo;
	
	$_SESSION['SesVveFotoLateral'.$Identificador] = $InsVehiculoVersion->VveFotoLateral;
	$_SESSION['SesVveFotoPosterior'.$Identificador] = $InsVehiculoVersion->VveFotoPosterior;
	$_SESSION['SesVveFotoCaracteristica'.$Identificador] = $InsVehiculoVersion->VveFotoCaracteristica;
...
//	Parametro9 = VcsNombre
	...
	if(!empty($InsVehiculoVersion->VehiculoVersionCaracteristica)){
		foreach($InsVehiculoVersion->VehiculoVersionCaracteristica as $DatVehiculoVersionCaracteristica){
			...
			$_SESSION['InsVehiculoVersionCaracteristica'.$Identificador]->MtdAgregarSesionObjeto(1,
			$DatVehiculoVersionCaracteristica->VvcId,
			$DatVehiculoVersionCaracteristica->VveId,
			$DatVehiculoVersionCaracteristica->VcsId,
			
			$DatVehiculoVersionCaracteristica->VvcDescripcion,
			$DatVehiculoVersionCaracteristica->VvcValor,
			$DatVehiculoVersionCaracteristica->VvcAnoModelo,
			date("d/m/Y H:i:s"),
			date("d/m/Y H:i:s"),
			$DatVehiculoVersionCaracteristica->VcsNombre
			);
