...
<?php
	if($POST_Borrar=="1"){
	?>
    <?php $_SESSION['SesSucFoto'.$Identificador] = ""; ?>
    <?php	
	}
?>
    <form method="post" enctype="multipart/form-data">
...
if (!empty($_FILES)) {
	...
	if($extension=="jpg" || $extension == "png" || $extension == "gif" || $extension == "jpeg" || $extension == "pdf" ){
		if (move_uploaded_file($tempFile,$targetFile)){
			...
			$_SESSION['SesSucFoto'.$Identificador] = $file_name;
...
}elseif(!empty($_SESSION['SesSucFoto'.$Identificador])){
	
	$extension = strtolower(pathinfo($_SESSION['SesSucFoto'.$Identificador], PATHINFO_EXTENSION));
	$nombre_base = basename($_SESSION['SesSucFoto'.$Identificador], '.'.$extension);  
?>
