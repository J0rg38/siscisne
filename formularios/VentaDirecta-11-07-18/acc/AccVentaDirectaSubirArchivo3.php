...
<?php
	if($POST_Borrar=="1"){
	?>
    <?php $_SESSION['SesVdiArchivoEntrega2'.$Identificador] = ""; ?>
    <?php	
	}
?>
...
if (!empty($_FILES)) {
	...
	if($extension=="jpg" || $extension == "png" || $extension == "gif" || $extension == "jpeg" || $extension == "pdf" ){
		if (move_uploaded_file($tempFile,$targetFile)){
			...
			$_SESSION['SesVdiArchivoEntrega2'.$Identificador] = $file_name;
...
}elseif(!empty($_SESSION['SesVdiArchivoEntrega2'.$Identificador])){
	
	$extension = strtolower(pathinfo($_SESSION['SesVdiArchivoEntrega2'.$Identificador], PATHINFO_EXTENSION));
	$nombre_base = basename($_SESSION['SesVdiArchivoEntrega2'.$Identificador], '.'.$extension);  
?>
