...
<tr>
  ...
  <?php
	if($POST_Borrar=="1"){
	?>
    <?php $_SESSION['SesVmvFoto'.$Identificador] = ""; ?>
    <?php	
	}
?>
...
if (!empty($_FILES)) {
	...
	$targetPath = '../../../subidos/vehiculo_movimiento_fotos/';
	...
	if($extension=="jpg" || $extension == "png" || $extension == "gif" || $extension == "jpeg"  || $extension == "pdf"){
		...
		if (move_uploaded_file($tempFile,$targetFile)){
			...
			$_SESSION['SesVmvFoto'.$Identificador] = $file_name;
			
			if($extension <> "pdf"){
				
				$mythumb = new thumb();
				$mythumb->loadImage('../../../subidos/vehiculo_movimiento_fotos/'.$file_name);
				$mythumb->crop(250, 250,'bottom');
				$mythumb->save('../../../subidos/vehiculo_movimiento_fotos/'.$nombre_base.'_thumb.'.$extension, 100);	
...
Vista Previa:<br />
	...
	if($extension <> "pdf"){
	?>
        <a target="_blank" href="../../../subidos/vehiculo_movimiento_fotos/<?php echo $nombre_base.".".$extension;?>">
        <img  src="../../../subidos/vehiculo_movimiento_fotos/<?php echo $nombre_base.".".$extension;?>" width="97" height="107" border="0" title="<?php echo $nombre_base.".".$extension;?>" />
        </a>   
    <?php
	}else{
	?>
...
}elseif(!empty($_SESSION['SesVmvFoto'.$Identificador])){
	
	$extension = strtolower(pathinfo($_SESSION['SesVmvFoto'.$Identificador], PATHINFO_EXTENSION));
	$nombre_base = basename($_SESSION['SesVmvFoto'.$Identificador], '.'.$extension);  
?>
...
Vista Previa:<br />
	...
	?>
        <a target="_blank" href="../../../subidos/vehiculo_movimiento_fotos/<?php echo $nombre_base.".".$extension;?>">
        <img  src="../../../subidos/vehiculo_movimiento_fotos/<?php echo $nombre_base.".".$extension;?>" width="95" height="113" border="0" title="<?php echo $nombre_base."_thumb.".$extension;?>" /></a>   
    <?php
	}else{
	?>
     <a target="_blank" href="../../../subidos/vehiculo_movimiento_fotos/<?php echo $nombre_base.".".$extension;?>">
