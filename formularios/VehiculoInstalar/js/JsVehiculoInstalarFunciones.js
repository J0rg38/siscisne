...
function FncValidar(){
	...
	var Total = $("#CmpTotal").val();
		...
		}else if(FncValidarPersonalHoraVehiculoInstalar()==false){
...
*/		
	...
	$('#BtnValidarVehiculoInstalar').on('click', function() {
		
		FncValidarPersonalHoraVehiculoInstalar();
		
	});
	
	
	
	$('#BtnVehiculoInstalarCalendario').on('click', function() {

		FncVehiculoInstalarCalendarioCargarFormulario();

	});
	
	$('#BtnVehiculoInstalarVerRestricciones').on('click', function() {

		FncVehiculoInstalarVerRestriccionesoCargarFormulario();
...
//	});
	...
	FncVehiculoInstalarMantenimientoKilometrajeEstablecer();
...
function FncVehiculoIngresoSimpleFuncion(InsVehiculoIngreso){
	...
	FncVehiculoInstalarHistorialListar();
...
function FncVehiculoInstalarMantenimientoKilometrajeEstablecer(){
	
	console.log("FncVehiculoInstalarMantenimientoKilometrajeEstablecer");
...
console.log(VehiculoMarcaId);
	$.getJSON("comunes/Vehiculo/JnPlanMantenimientoKilometraje.php?VehiculoMarcaId="+VehiculoMarcaId,{}, function(j){
		...
		$('select#CmpVehiculoInstalarPresupuestoMantenimientoKilometraje').html(options);
...
function FncVehiculoInstalarCalendarioCargarFormulario(){
	
	//tb_show(this.title,'principal2.php?Mod=VehiculoInstalar&Form=VerCalendarioFull&Dia=1&placeValuesBeforeTB_=savedValues&TB_iframe=true&height=620&width=890&modal=true',this.rel);		
	FncCargarVentanaFullv3('principal2','VehiculoInstalar','VerCalendarioFull','');
...
function FncVehiculoInstalarVerRestriccionesoCargarFormulario(){
	
	//tb_show(this.title,'principal2.php?Mod=VehiculoInstalar&Form=VerRestricciones&Dia=1&placeValuesBeforeTB_=savedValues&TB_iframe=true&height=280&width=480&modal=true',this.rel);
	
	FncCargarVentanaFullv3('principal2','VehiculoInstalar','VerRestricciones','');		

}

function FncValidarPersonalHoraVehiculoInstalar(){
	...
	/*	if(PersonalMecanico==""){
			...
			dhtmlx.alert({
				...
				$.ajax({
					type: 'POST',
					dataType: "json",
					url: 'formularios/VehiculoInstalar/acc/AccValidarVehiculoInstalar.php',
					data: 'FechaProgramada='+FechaProgramada+
					'&HoraProgramada='+(HoraProgramada)+
					'&Sucursal='+(Sucursal)+
					'&PersonalMecanico='+PersonalMecanico,
					success: function(InsVehiculoInstalar){
						
						if(InsVehiculoInstalar['respuesta']=="2"){

							dhtmlx.confirm("Se ha alcanzado el total de citas permitidos en este horario. ("+InsVehiculoInstalar['citas']+" de "+InsVehiculoInstalar['limite']+" citas). ¿Desea continuar de todas maneras?", function(result){
								if(result==true){		
									respuesta = true;
								}else{
									respuesta = false;
							...
							var saldo = InsVehiculoInstalar['limite'] - InsVehiculoInstalar['citas']

							dhtmlx.alert({
								title:"Aviso",
								type:"alert",
								text:"VehiculoInstalar disponible en este horario ("+saldo+" de "+InsVehiculoInstalar['limite']+" citas)",
								callback: function(result){
...
//							});
						...
						//if(InsVehiculoInstalar.CitId !=null){
//								
...
function FncCargarPersonalHorario(){
	...
	if(FechaProgramada != "" && HoraProgramada=="" && PersonalMecanico==""){
		...
		$.ajax({
			type: 'POST',
			dataType: "json",
			url: 'formularios/VehiculoInstalar/CapPersonalHorario.php',
			data: 'FechaProgramada='+FechaProgramada+
			'&HoraProgramada='+(HoraProgramada)+
			'&PersonalMecanico='+PersonalMecanico,
...
function FncVehiculoInstalarHistorialListar(){
console.log("FncVehiculoInstalarHistorialListar");
	var Identificador = $('#Identificador').val();

	var VehiculoIngresoVIN = $("#CmpVehiculoIngresoVIN").val();
	var VehiculoIngresoId = $("#CmpVehiculoIngresoId").val();	

	$('#CapVehiculoInstalarHistorialAccion').html('Cargando...');
	
	$.ajax({
		type: 'POST',
		url: 'formularios/VehiculoInstalar/FrmVehiculoInstalarHistorial.php',
		data: 'VehiculoIngresoVIN='+VehiculoIngresoVIN+'&VehiculoIngresoId='+VehiculoIngresoId,
		success: function(html){
			$('#CapVehiculoInstalarHistorialAccion').html('Listo');	
			$("#CapVehiculoInstalarHistoriales").html("");
			$("#CapVehiculoInstalarHistoriales").append(html);
		}
	});
