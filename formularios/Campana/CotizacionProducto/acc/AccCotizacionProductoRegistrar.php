...
if(!empty($ArrCotizacionProductoDetalles)){
	...
	if(!empty($InsCotizacionProducto->CprPorcentajeDescuento)){
		...
		$InsCotizacionProductoDetalle1->CrdEstado = $DatCotizacionProductoDetalle->Parametro21;
...
//	}
	...
	switch($GET_Origen){
		...
		case "TallerPedido":
			...
			$InsCotizacionProducto->EinVIN = $InsFichaIngreso->EinVIN;
...
if($InsCotizacionProducto1->MonId<>$EmpresaMonedaId ){
			...
			if(!empty($InsCotizacionProducto1->CprPorcentajeDescuento)){
				...
				$DetallePrecioDescuento =  $DetallePrecio - ($DetallePrecio * ($InsCotizacionProducto1->CprPorcentajeDescuento/100));
				
				$DetalleDescuento = ($DetalleImporte * ( $InsCotizacionProducto1->CprPorcentajeDescuento/100));
				$DetalleImporteFinal = $DetalleImporte - $DetalleDescuento;
		...
		//	echo $DatCotizacionProductoDetalle->CrdImporte;
		//	echo "<br>";
...
//deb($DatCotizacionProductoDetalle->CrdImporte);
							/*$_SESSION['InsCotizacionProductoDetalle'.$Identificador]->MtdAgregarSesionObjeto(1,
							NULL,
	...
	<?php
		...
		case "ConsultaProducto":
			...
			//						Parametro21 = CrdEstado
						...
						if(!empty($InsCotizacionProducto->CprMargenUtilidad)){
							$Precio = $Costo + $Costo * ($InsCotizacionProducto->CprMargenUtilidad/100);
						}
						
						if(!empty($InsCotizacionProducto->CprFlete)){
							$ValorVenta = $Precio = $Precio + $Precio * ($InsCotizacionProducto->CprFlete/100);
						}
...
function FncNuevo(){
	...
	$InsCotizacionProducto->CprMargenUtilidad = $EmpresaRepuestoMargenUtilidad;
	$InsCotizacionProducto->CprFlete = $EmpresaRepuestoFlete;
