...
$InsNotaDebito->BolId = $GET_id;
$InsNotaDebito->BtaId = $GET_ta;
...
$().ready(function() {
	...
	$('#CapNotaDebitoAccion').html('Conectando con SUNAT para procesar archivo xml...');	
	
	$.ajax({
		type: 'GET',
		dataType: 'json',
		url: 'acc/AccNotaDebitoProcesarXMLv2.php',
		data: 'Id=<?php echo $InsNotaDebito->BolId;?>&Ta=<?php echo $InsNotaDebito->BtaId;?>',
		success: function(respuesta){
			//alert(respuesta['CodigoRespuesta']);
			console.log(respuesta['CodigoRespuesta']);
			
			switch(respuesta['CodigoRespuesta']){
				
				case "P101":
				
					$('#CapNotaDebitoAccion').html('Archivo xml procesado correctamente. '+respuesta['MensajeRespuesta']);								
				...
				default:
					...
					$('#CapNotaDebitoAccion').html('Ha ocurrido un error procesando el archivo xml, proceso cancelado.');
