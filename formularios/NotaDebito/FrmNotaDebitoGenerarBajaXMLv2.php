...
$().ready(function() {
	...
	$.ajax({
		type: 'GET',
		dataType: 'json',
		url: 'acc/AccNotaDebitoGenerarBajaXMLv2.php',
			...
			switch(respuesta['CodigoRespuesta']){
				case "1":
					...
					?>	
						...
						window.setTimeout("FncNotaDebitoProcesarBajaXML('"+respuesta['Id']+"');",2500);
					<?php	
					}else{
					?>
						$('#CapNotaDebitoAccion').html('Archivo xml de baja generado correctamente...');
...
function FncNotaDebitoProcesarBajaXML(oId){
	
	FncPopUp("FrmNotaDebitoProcesarBajaXMLv2.php?Id="+oId+"&Procesar=<?php echo $GET_Procesar?>&EnviarSUNAT=<?php echo $GET_EnviarSUNAT;?>&P=1",0,0,1,0,0,1,0,350,150);
