...
//cbc:ReferenceDate
$ReferenceDate = $domtree->createElement("cbc:ReferenceDate",FncCambiaFechaAMysql($InsNotaDebito->NdbFechaEmision));
...
$AccountingSupplierParty = $xmlRoot->appendChild($AccountingSupplierParty);
	...
	$Party = $AccountingSupplierParty->appendChild($Party);
		...
		$VoidedDocumentsLine = $xmlRoot->appendChild($VoidedDocumentsLine);	
			...
			$DocumentTypeCode = $domtree->createElement("cbc:DocumentTypeCode","08");
			$DocumentTypeCode = $VoidedDocumentsLine->appendChild($DocumentTypeCode);	
			
			//cbc:DocumentSerialID
			$DocumentSerialID = $domtree->createElement("sac:DocumentSerialID",$InsNotaDebito->NdtNumero);
			$DocumentSerialID = $VoidedDocumentsLine->appendChild($DocumentSerialID);	
			
			//cbc:DocumentNumberID
			$DocumentNumberID = $domtree->createElement("sac:DocumentNumberID",$InsNotaDebito->NdbId);
...
$InsResumenBaja = new ClsResumenBaja();
$InsResumenBaja->NdbId = $InsNotaDebito->NdbId;
$InsResumenBaja->NdtId = $InsNotaDebito->NdtId;
...
//$InsNotaDebito->MtdEditarNotaDebitoDato("NdbSunatRespuestaBajaId",$BajaNumeracion,$InsNotaDebito->NdbId,$InsNotaDebito->NdtId);
