...
function FncEntregaVentaVehiculoActualizarPendienteSeleccionados(){
	...
	if(seleccionados==""){
		...
		if(confirm("¿Realmente desea marcar como Pendiente la(s) asignaciones de vin?")){
			...
			if(actualizar){
				$("#Acc").val("EntregaVentaVehiculoActualizarPendiente");
				$("#FrmListado").submit();	
			}else{
...
function FncEntregaVentaVehiculoActualizarEmitidoSeleccionados(){
	...
	if(seleccionados==""){
		...
		if(confirm("¿Realmente desea marcar como Aprobado la(s) asignaciones de vin?")){
			...
			if(actualizar){
				//$("#Acc").val("EntregaVentaVehiculoVentaAnularEnvioFacturacion");
				$("#Acc").val("EntregaVentaVehiculoActualizarEmitido");
				$("#FrmListado").submit();	
			}else{
				alert("Uno o mas de los elementos seleccionados no puede ser enviado, verifique sus ESTADOS.");
			}
...
function FncEntregaVentaVehiculoActualizarAnuladoSeleccionados(){
				...
				if(actualizar){
					...
					if(actualizar2){	
						...
						if(actualizar3){	
							...
							$("#Acc").val("EntregaVentaVehiculoActualizarAnulado");
							$("#FrmListado").submit();
...
function FncImprmir(oId){
	...
	FncPopUp('formularios/EntregaVentaVehiculo/FrmEntregaVentaVehiculoImprimir.php?Id='+oId+'&P=1',0,0,1,0,0,1,0,screen.height,screen.width);
...
function FncVistaPreliminar(oId){
	...
	FncPopUp('formularios/EntregaVentaVehiculo/FrmEntregaVentaVehiculoImprimir.php?Id='+oId,0,0,1,0,0,1,0,screen.height,screen.width);
...
//function FncEntregaVentaVehiculoCargarFormulario(oForm,oEntregaVentaVehiculoId){
//		
//	FncCargarVentana("EntregaVentaVehiculo",oForm,oEntregaVentaVehiculoId);
//
//}
