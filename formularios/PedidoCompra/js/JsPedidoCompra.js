...
//					case "1":
	document.getElementById("FrmListado").action = "formularios/PedidoCompra/acc/AccPedidoCompraGenerarExcel.php";
//					break;
//					
//					case "2":
//	document.getElementById("FrmListado").action = "formularios/PedidoCompra/acc/AccPedidoCompraGenerarExcel2.php";
//					break;
//				
//				}
//				
...
function FncListadoImprimir(){
	...
	document.getElementById("FrmListado").action = "formularios/PedidoCompra/FrmPedidoCompraListadoImprimir.php"
...
//
//			if(actualizar){
//				$("#Acc").val("EnviarPedidoCompraContabilidad");
//				$("#FrmListado").submit();	
//			}else{
		...
		if(confirm("¿Realmente desea GENERAR la ORDEN DE COMPRA con los elementos seleccionados?")){
			...
			if(actualizar){
				...
				if(actualizar2){
					...
					$("#FrmListado").attr("action","principal.php?Mod=OrdenCompra&Form=Registrar&Ori=PedidoCompra");
					$("#FrmListado").submit();	
...
function FncProveedorLineaCredito(oId){

	tb_show(this.title,'principal2.php?Mod=Proveedor&Form=VerLineaCredito&Dia=1&Id='+oId+'&placeValuesBeforeTB_=savedValues&TB_iframe=true&height=620&width=890&modal=true',this.rel);	
...
function FncPedidoCompraEnviarCorreo(oId,oAprobado,oLineaCreditoActiva){
	
	if(oAprobado == "1"){
		
		if(oLineaCreditoActiva == "1"){
			
			window.location = "principal.php?Mod=PedidoCompra&Form=EnviarCorreo&Id="+oId;	
			...
			dhtmlx.alert({
				title:"Aviso",
				type:"alert-error",
				text:"No hay linea de credito para hacer este pedido",
			...
			dhtmlx.alert({
					title:"Aviso",
					type:"alert-error",
					text:"Este pedido requiere aprobacion",
					callback: function(result){
...
function FncPedidoCompraSolicitarAutorizacion(oId,oAprobado,oLineaCreditoActiva){
	
	if(oAprobado == "2"){
		
//		if(oLineaCreditoActiva == "1"){
			
			window.location = "principal.php?Mod=PedidoCompra&Form=SolicitarAutorizacion&Id="+oId;	
...
//		}
			...
			dhtmlx.alert({
					title:"Aviso",
					type:"alert-error",
					text:"Este pedido ya se encuentra aprobado",
					callback: function(result){
...
function FncDescargarExcel(oId){
		
	FncPopUp('formularios/PedidoCompra/acc/AccPedidoCompraGenerarExcelFormatoGM.php?Id='+oId,0,0,1,0,0,1,0,screen.height,screen.width);
