...
if ($TotalLineas < 10 and empty($InsFactura->OvvId)) {
	...
	$TotalLineasFaltantes = 12 - $TotalLineas;
...
//DETALLE DE CREDITO
if ($InsFactura->NpaNombre == 'CREDITO') {
	if ($InsFactura->FacSpot == 1) {
		$DetraccionMonto = round(($InsFactura->FacTotal) * 0.12);
		//SOLO EN CASO DE DOLARES
		$DetraccionMonto = $DetraccionMonto * 3.790;
		$DetraccionMonto = round($DetraccionMonto);

		$NetoPagar = ($InsFactura->FacTotal) - ($DetraccionMonto / 3.790);

		$pdf->Cell(5, 2, "", $lineas, 1, "C");
		$pdf->Cell(65, 5, "FORMA DE PAGO Y CUOTAS", 1, 1, "C");

		$pdf->Cell(20, 5, "Credito", 1, 0, "C");
		$pdf->Cell(20, 5, "", 1, 0, "C");
		$pdf->Cell(10, 5, $InsFactura->MonSimbolo, 1, 0, "C");
		$pdf->Cell(15, 5, number_format($NetoPagar, 2), 1, 1, "C");

		$pdf->Cell(20, 5, "Cuota001", 1, 0, "C");
		$pdf->Cell(20, 5, $InsFactura->FacFechaVencimiento, 1, 0, "C");
		$pdf->Cell(10, 5, $InsFactura->MonSimbolo, 1, 0, "C");
		$pdf->Cell(15, 5, number_format($NetoPagar, 2), 1, 0, "C");
	} else {
		...
		$pdf->Cell(20, 5, "", 1, 0, "C");
		$pdf->Cell(10, 5, $InsFactura->MonSimbolo, 1, 0, "C");
		$pdf->Cell(15, 5, number_format($InsFactura->FacTotal, 2), 1, 1, "C");

		$pdf->Cell(20, 5, "Cuota001", 1, 0, "C");
		$pdf->Cell(20, 5, $InsFactura->FacFechaVencimiento, 1, 0, "C");
		$pdf->Cell(10, 5, $InsFactura->MonSimbolo, 1, 0, "C");
		$pdf->Cell(15, 5, number_format($InsFactura->FacTotal, 2), 1, 0, "C");
	}
} else {
}
...
if ($InsFactura->RegId == "REG-10000") {
	...
	$pdf->Cell(120, 5, "", $lineas, 0, 'C');
	$pdf->SetFont('times', 'B', 8);
	$pdf->Cell(35, 5, "DETRACCION (" . $InsFactura->FacRegimenPorcentaje . ") %", $lineas, 0, 'R');
	$pdf->SetFont('times', 'N', 8);
	$pdf->Cell(10, 5, $InsFactura->MonSimbolo, $lineas, 0, 'L');
	$pdf->Cell(25, 5, number_format($InsFactura->FacRegimenMonto, 2), 1, 1, 'R');

	$pdf->Cell(120, 5, "", $lineas, 0, 'C');
	$pdf->SetFont('times', 'B', 8);
	$pdf->Cell(35, 5, "IMPORTE A PAGAR", $lineas, 0, 'R');
	$pdf->SetFont('times', 'N', 8);
	$pdf->Cell(10, 5, $InsFactura->MonSimbolo, $lineas, 0, 'L');
	$pdf->Cell(25, 5, number_format($InsFactura->FacTotalReal, 2), 1, 1, 'R');
}
...
if ($InsFactura->RegId != "") {
	...
	if ($InsFactura->RegId == "REG-10000") {
		...
		$pdf->SetFont('times', 'B', 8);
		$pdf->Cell(190, 5, "Detraccion:", 0, 1, 'L');

		$pdf->SetFont('times', 'N', 8);
		$pdf->Cell(190, 5, "Porcentaje: " . number_format($InsFactura->FacRegimenPorcentaje, 2) . "%", 0, 1, 'L');
		$pdf->Cell(190, 5, "Monto detraccion: " . number_format($InsFactura->FacRegimenMonto, 2), 0, 1, 'L');
		$pdf->Cell(190, 5, "Cuenta: " . $EmpresaCuentaDetraccion, 0, 1, 'L');
	}
} else {
	...
	if ($InsFactura->FacSpot == 1) {
		...
		// $pdf->SetFont('times', 'N', 8);
		// $pdf->Cell(10,5,$InsFactura->MonSimbolo,$lineas,0,'L');
		// $pdf->Cell(25,5,number_format($NetoPagar,2),1,1,'R');
		...
		$pdf->SetFont('times', 'N', 8);
		$pdf->Cell(95, 5, "% detraccion: 12.00", 0, 0, 'L');
		$pdf->SetFont('times', 'B', 9);
		$pdf->Cell(95, 5, "Monto detraccion: S/ " . number_format($DetraccionMonto, 2), 0, 1, 'L');
		$pdf->SetFont('times', 'N', 7);
		$pdf->Cell(190, 5, "Descripcion: Operacion sujeta al sistema de pagos obligaciones tributarias del Banco de la Nacion 00101556808", 0, 1, 'L');
