...
*/
	...
	if (!empty($ResFacturaDetalle['Datos'])) {
		foreach ($ResFacturaDetalle['Datos'] as $DatSesionObjeto) {
			...
			if ($InsFactura->MonId <> $EmpresaMonedaId) {
				...
				$DatSesionObjeto->Parametro4 = round($DatSesionObjeto->Parametro4 * $InsFactura->FacTipoCambio, 6);
				$DatSesionObjeto->Parametro6 = round($DatSesionObjeto->Parametro6 * $InsFactura->FacTipoCambio, 6);

				$DatSesionObjeto->Parametro19 = round($DatSesionObjeto->Parametro19 * $InsFactura->FacTipoCambio, 6);
				$DatSesionObjeto->Parametro20 = round($DatSesionObjeto->Parametro20 * $InsFactura->FacTipoCambio, 6);
				$DatSesionObjeto->Parametro21 = round($DatSesionObjeto->Parametro21 * $InsFactura->FacTipoCambio, 6);
				$DatSesionObjeto->Parametro25 = round($DatSesionObjeto->Parametro25 * $InsFactura->FacTipoCambio, 6);
			} else {
				$DatSesionObjeto->Parametro4 = round($DatSesionObjeto->Parametro4, 2);
				$DatSesionObjeto->Parametro6 = round($DatSesionObjeto->Parametro6, 2);

				$DatSesionObjeto->Parametro19 = round($DatSesionObjeto->Parametro19, 2);
				$DatSesionObjeto->Parametro20 = round($DatSesionObjeto->Parametro20, 2);
				$DatSesionObjeto->Parametro21 = round($DatSesionObjeto->Parametro21, 2);
				$DatSesionObjeto->Parametro25 = round($DatSesionObjeto->Parametro25, 2);
			}
			...
			if ($InsFacturaDetalle1->FdeEliminado == 1) {
				...
				//deb($InsFacturaDetalle1->FdeValorVenta);
			}
		}
	}

	//deb($InsFactura->FacTotalDescuento);
	/*
	deb($InsFactura->FacTotalGravado);
	...
	/*
	deb($InsFactura->FacSubTotal);
	deb($InsFactura->FacImpuesto);
	deb($InsFactura->FacTotal);*/
...
*/
			...
			$_SESSION['InsFacturaDetalle' . $Identificador]->MtdAgregarSesionObjeto(
				1,
	...
	if (!empty($InsFactura->FacturaAlmacenMovimiento)) {
		foreach ($InsFactura->FacturaAlmacenMovimiento as $DatFacturaAlmacenMovimiento) {
			...
			$_SESSION['InsFacturaAlmacenMovimiento' . $Identificador]->MtdAgregarSesionObjeto(
				1,
