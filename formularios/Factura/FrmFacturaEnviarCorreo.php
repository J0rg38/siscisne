...
$NOMBRE = 'R-'.$EmpresaCodigo.'-01-'.$InsFactura->FtaNumero.'-'.$InsFactura->FacId;
//$ARCHIVO = $NOMBRE.'.xml';
?>
...
$().ready(function() {
	...
	$('#CapFacturaAccion').html('Conectando con GMAIL para enviar correo electronico...');	
	
	$.ajax({
		type: 'GET',
		dataType: 'json',
		url: 'acc/AccFacturaEnviarCorreo.php',
		...
		success: function(respuesta){
			...
			switch(respuesta['CodigoRespuesta']){
				...
				case "1":
					...
					$('#CapFacturaAccion').html('Correo electronico enviado correctamente. '+respuesta['MensajeRespuesta']);	
				...
				case "2":
					...
					$('#CapFacturaAccion').html('No se encontro direccion de correo electronico a enviar. '+respuesta['MensajeRespuesta']);						
				
				break;
				
				default:
				
					$('#CapFacturaAccion').html('Ha ocurrido un error enviando el correo electronico, proceso cancelado.');
		...
		error: function(respuesta){
			...
			$('#CapFacturaAccion').html('Ha ocurrido un error interno.');
