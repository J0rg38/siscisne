	...
	if(seleccionados==""){
		...
		if(confirm("¿Realmente desea ELIMINAR los elementos seleccionados?")){
			...
			$('input[type=checkbox]').each(function () {
				...
				if($(this).attr('name')=="cmp_seleccionar[]"){
					if($(this).is(':checked')){
						if($(this).attr('estado')=="3" || $(this).attr('estado')=="4"){
							eliminar = false;
							return false;
						}
					}
			...
			if(eliminar){
				...
				alert("Uno o mas de los elementos seleccionados no puede ser ELIMINADO, verifique sus ESTADOS.");
			}	
...
function FncMarcarEnviadoSeleccionados(){
	...
	if(seleccionados==""){
		...
		if(confirm("¿Realmente desea MARCAR como ENVIADO los elementos seleccionados?")){
			...
			if(actualizar){
				if(actualizar2){
					...
					alert("Uno o mas de los elementos seleccionados no puede ser ENVIADO, no se encontron PEDIDOS en la ORDEN DE COMPRA.");
				}
			}else{
