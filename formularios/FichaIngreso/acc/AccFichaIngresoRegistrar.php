...
if (isset($_POST['BtnGuardar_x']) or ($_POST['Guardar'] ?? '') == "1") {
	...
	$InsFichaIngreso->FinId2 = $_POST['CmpId2'];
	...
	$InsFichaIngreso->FinMontoPresupuesto = str_replace(",", "", (empty($_POST['CmpMontoPresupuesto'] ?? '') ? 0 : $_POST['CmpMontoPresupuesto'] ?? ''));
	...
	$InsFichaIngreso->FinModalidad = 0;
	...
	switch ($GET_Origen) {
		...
		case "Cita":
			...
			if (!empty($GET_CitId)) {
				...
				$InsFichaIngreso->FinTelefono = $InsCita->CliCelular;
				$InsFichaIngreso->FinClienteEmail = $InsCita->CliEmail;
		...
		case "CotizacionProducto":
			...
			if (!empty($InsCotizacionProducto->CotizacionProductoDetalle)) {
				foreach ($InsCotizacionProducto->CotizacionProductoDetalle as $DatCotizacionProductoDetalle) {
					...
					$_SESSION['InsFichaIngresoProducto' . $InsFichaIngresoModalidad->MinSigla . $Identificador]->MtdAgregarSesionObjeto(
						1,
						NULL,
			...
			if (!empty($InsCotizacionProducto->CotizacionProductoPlanchado)) {
				foreach ($InsCotizacionProducto->CotizacionProductoPlanchado as $DatCotizacionProductoPlanchado) {
					...
					$_SESSION['InsFichaIngresoTarea' . $InsFichaIngresoModalidad->MinSigla . $Identificador]->MtdAgregarSesionObjeto(
						1,
						NULL,
						NULL,
