<?php
// Función helper para acceder de forma segura a $_POST
...
if (isset($_POST['BtnGuardar_x']) or (isset($_POST['Guardar']) && $_POST['Guardar'] == "1")) {
	...
	$InsFichaIngreso->FinFechaVenta = getPostValue('CmpVehiculoPromocionFechaVenta', '');
	$InsFichaIngreso->FinCantidadMantenimientos = getPostValue('CmpVehiculoPromocionCantidadMantenimientos', '');
	...
	$InsFichaIngreso->MinId = getPostValue('CmpModalidad', '');

	$InsFichaIngreso->TreId = getPostValue('CmpTipoReparacion', '');
	...
	$InsFichaIngreso->FinInformeTecnicoMantenimiento = getPostValue('CmpMantenimiento', '');
	$InsFichaIngreso->FinInformeTecnicoRevision = getPostValue('CmpRevision', '');
	$InsFichaIngreso->FinInformeTecnicoDiagnostico = getPostValue('CmpDiagnostico', '');

	$InsFichaIngreso->FinSalidaFecha = FncCambiaFechaAMysql(getPostValue('CmpSalidaFecha', ''), true);
	$InsFichaIngreso->FinSalidaHora = getPostValue('CmpSalidaHora', '');
	$InsFichaIngreso->FinPlaca = $_POST['CmpVehiculoIngresoPlaca'];
	$InsFichaIngreso->FinSalidaObservacion = getPostValue('CmpSalidaObservacion', '');
	...
	$InsFichaIngreso->CliNombreCompleto = getPostValue('CmpClienteNombre', '');
	$InsFichaIngreso->CliNombre = getPostValue('CmpClienteNombre', '');
	$InsFichaIngreso->CliApellidoPaterno = getPostValue('CmpClienteApellidoPaterno', '');
	$InsFichaIngreso->CliApellidoMaterno = getPostValue('CmpClienteApellidoMaterno', '');
	...
	foreach ($ArrModalidadIngresos as $DatModalidadIngreso) {
		if (!empty($DatModalidadIngreso->MinId)) {
			...
			if (getPostValue('CmpModalidadId_' . $DatModalidadIngreso->MinSigla) == "MIN-10001") {
	...
	foreach ($ArrModalidadIngresos as $DatModalidadIngreso) {
		...
		$InsFichaIngresoModalidad1->FimId = getPostValue('CmpFichaIngresoModalidadId_' . $DatModalidadIngreso->MinSigla, '');
		$InsFichaIngresoModalidad1->MinId = getPostValue('CmpModalidadId_' . $DatModalidadIngreso->MinSigla, '');
		$InsFichaIngresoModalidad1->FimObsequio = !empty(getPostValue('CmpFichaIngresoModalidadObsequio_' . $DatModalidadIngreso->MinSigla)) ? 1 : 2;
		...
		//Parmaetro19 = FipPrecio
		//Parmaetro20 = FipImporte
		...
		*/
		...
		if (!empty($ArrSesionObjetos)) {
			foreach ($ArrSesionObjetos as $DatSesionObjeto) {
				...
				$_SESSION['InsFichaIngresoSuministro' . $DatModalidadIngreso->MinSigla . $Identificador]->MtdEditarSesionObjeto(
		...
		if ($DatModalidadIngreso->MinId == "MIN-10001") {
			...
			//echo count($ArrSesionObjetos)." TOTAL";
			//$ResPlanMantenimiento = $InsPlanMantenimiento->MtdObtenerPlanMantenimientos(NULL,NULL,NULL,'PmaId','ASC',1,NULL,NULL,$InsFichaIngreso->VmoId) ;
			//$ArrPlanMantenimientos = $ResPlanMantenimiento['Datos'];				
			...
			foreach ($ArrSesionObjetos as $DatSesionObjeto) {
				...
				if (!empty($_POST['CmpPlanMantenimientoDetalleAccion_' . $DatSesionObjeto->Parametro3])) {
					...
					$_SESSION['InsFichaIngresoMantenimiento' . $DatModalidadIngreso->MinSigla . $Identificador]->MtdEditarSesionObjeto(
						$DatSesionObjeto->Item,
						1,
	...
	//deb($ModalidadMantenimiento);
	//deb($ModalidadMantenimientoMarcado);
	////deb($InsPlanMantenimiento->PmaId);
	//deb($InsFichaIngreso->FinMantenimientoKilometraje);	
	if ($ModalidadMantenimiento and $ModalidadMantenimientoMarcado) {
...
function FncCargarDatos()
{
	...
	if ($GET_Form <> "Ver") {
		if ($InsFichaIngreso->FinEstado == 73) {
			$InsFichaIngreso->MtdActualizarEstadoFichaIngreso($InsFichaIngreso->FinId, 74, false); //OJOOO		
		}
	}
			...
			if (!empty($DatFichaIngresoModalidad->FichaIngresoTarea)) {
				foreach ($DatFichaIngresoModalidad->FichaIngresoTarea as $DatFichaIngresoTarea) {
					...
					if (!empty($DatFichaIngresoTarea->MinSigla)) { //AUX
						$_SESSION['InsFichaIngresoTarea' . $DatFichaIngresoTarea->MinSigla . $Identificador]->MtdAgregarSesionObjeto(
							1,
			...
			if (!empty($DatFichaIngresoModalidad->FichaIngresoManoObra)) {
				foreach ($DatFichaIngresoModalidad->FichaIngresoManoObra as $DatFichaIngresoManoObra) {
					...
					if (!empty($DatFichaIngresoManoObra->MinSigla)) { //AUX
						...
						$_SESSION['InsFichaIngresoManoObra' . $DatFichaIngresoManoObra->MinSigla . $Identificador]->MtdAgregarSesionObjeto(
							1,
			...
			if (!empty($DatFichaIngresoModalidad->FichaIngresoProducto)) {
				foreach ($DatFichaIngresoModalidad->FichaIngresoProducto as $DatFichaIngresoProducto) {
					...
					if (!empty($DatFichaIngresoProducto->MinSigla)) { //AUX
						...
						$_SESSION['InsFichaIngresoProducto' . $DatFichaIngresoProducto->MinSigla . $Identificador]->MtdAgregarSesionObjeto(
							1,
							$DatFichaIngresoProducto->FipId,
							$DatFichaIngresoProducto->ProId,
							$DatFichaIngresoProducto->ProNombre,
			...
			if (!empty($DatFichaIngresoModalidad->FichaIngresoSuministro)) {
				foreach ($DatFichaIngresoModalidad->FichaIngresoSuministro as $DatFichaIngresoSuministro) {
					...
					if (!empty($DatFichaIngresoSuministro->MinSigla)) { //AUX
						$_SESSION['InsFichaIngresoSuministro' . $DatFichaIngresoSuministro->MinSigla . $Identificador]->MtdAgregarSesionObjeto(
							1,
			...
			if (!empty($DatFichaIngresoModalidad->FichaIngresoMantenimiento)) {
				foreach ($DatFichaIngresoModalidad->FichaIngresoMantenimiento as $DatFichaIngresoMantenimiento) {
					...
					if (!empty($DatFichaIngresoMantenimiento->MinSigla)) { //AUX
						$_SESSION['InsFichaIngresoMantenimiento' . $DatFichaIngresoMantenimiento->MinSigla . $Identificador]->MtdAgregarSesionObjeto(
							1,
							...
							$DatFichaIngresoMantenimiento->ProId
						);
					}
				}
			}
