...
<tr>
  ...
  <td width="54%" align="center" valign="top"><span class="EstReporteTitulo">SEGUIMIENTO DE ORDENES DE TRABAJO C/ COTIZACION
...
<?php }?>
        ...
        <tr>
          ...
          <th width="12%">TIPO PEDIDO</th>
          <th width="7%">FECHA PEDIDO</th>
          <th width="4%">O.T.</th>
...
<?php
	switch($DatFichaIngresoCotizacionProductoSeguimiento->OcoTipo){
		case "STK":
	?>
     NORMAL
    <?php		
...
?>
		  ...
		  <?php //echo $DatFichaIngresoCotizacionProductoSeguimiento->OcoTipo;  ?>
          
          </td>
          <td class="<?php echo (($c%2==0)?'EstTablaReporteActivo':'EstTablaReporteInactivo');?>"  align="right" >
          <?php echo $DatFichaIngresoCotizacionProductoSeguimiento->OcoFecha;  ?>
         ...
         <a href="javascript:FncPopUp('../../formularios/FichaIngreso/FrmFichaIngresoImprimirFT.php?Id=<?php echo $DatFichaIngresoCotizacionProductoSeguimiento->FinId;?>&ImprimirCodigo=1',0,0,1,0,0,1,0,screen.height,screen.width);"><?php echo $DatFichaIngresoCotizacionProductoSeguimiento->FinId;  ?>  </a>
          ...
          <td class="<?php echo (($c%2==0)?'EstTablaReporteActivo':'EstTablaReporteInactivo');?>" align="right" ><?php echo $DatFichaIngresoCotizacionProductoSeguimiento->ProCodigoOriginal;  ?></td>
          <td class="<?php echo (($c%2==0)?'EstTablaReporteActivo':'EstTablaReporteInactivo');?>" align="right" ><?php echo $DatFichaIngresoCotizacionProductoSeguimiento->ProNombre;  ?></td>
          <td class="<?php echo (($c%2==0)?'EstTablaReporteActivo':'EstTablaReporteInactivo');?>" align="right" >
		  
		  <?php echo number_format($DatFichaIngresoCotizacionProductoSeguimiento->PcdCantidad,2);  ?>
		  ...
		  <?php
		  if(!empty($DatFichaIngresoCotizacionProductoSeguimiento->AmoId)){
			?>
            	Atendido por Proveedor: 
                
                <?php echo $DatFichaIngresoCotizacionProductoSeguimiento->AmoComprobanteFecha;?>
           
           <?php
		  }elseif(!empty($DatFichaIngresoCotizacionProductoSeguimiento->PldId)){
		   ?>    
               Tiene Despacho:  <?php echo $DatFichaIngresoCotizacionProductoSeguimiento->PleFecha;?>
                 
            <?php
		  }elseif(!empty($DatFichaIngresoCotizacionProductoSeguimiento->PcdBOFecha)){
			?>
            En Back Order: 
            <?php echo $DatFichaIngresoCotizacionProductoSeguimiento->PcdBOEstado;?> - 
            	
				<?php echo $DatFichaIngresoCotizacionProductoSeguimiento->PcdBOFecha;?>
                
            <?php 
		  }elseif(!empty($DatFichaIngresoCotizacionProductoSeguimiento->OcoFechaLlegadaEstimada)){
		?>
			<?php
				$FechaEstimadaLlegada = strtotime ( '+3 days' , strtotime ( FncCambiaFechaAMysql($DatFichaIngresoCotizacionProductoSeguimiento->OcoFechaLlegadaEstimada)) ) ;
				$FechaEstimadaLlegada = date ('d/m/Y' , $FechaEstimadaLlegada );
			?> 
               Llegada estimada: <?php echo $FechaEstimadaLlegada;  ?>        
        <?php
        ...
        ?>
          <tr>
            <td align="right">&nbsp;</td>
            <td colspan="12" align="right">* Las fechas de llegada de los pedidos por importacion son referenciales y no reflejan con exactitud la llegada del repuesto.</td>
          </tr>
		</tbody>
