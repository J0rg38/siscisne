...
if (!empty($_FILES)) {
	...
	$targetPath = '../../../subidos/clientepago_fotos/';
	...
	if($extension=="jpg" || $extension == "png" || $extension == "gif" || $extension == "jpeg"){
		if (move_uploaded_file($tempFile,$targetFile)){
			...
			$_SESSION['SesCpaFoto'.$Identificador] = $file_name;
			
			$mythumb = new thumb();
			$mythumb->loadImage('../../../subidos/clientepago_fotos/'.$file_name);
			$mythumb->crop(250, 250,'bottom');
			$mythumb->save('../../../subidos/clientepago_fotos/'.$nombre_base.'_thumb.'.$extension, 100);
			
			
//			$mythumb = new thumb();
//			$mythumb->loadImage('../../../subidos/clientepago_fotos/'.$file_name);
//			$mythumb->crop(21, 20,'bottom');
//			$mythumb->save('../../../subidos/clientepago_fotos/'.$nombre_base.'_thumb2.'.$extension, 20);

?>
Vista Previa:<br />
  
  <img  src="../../../subidos/clientepago_fotos/<?php echo $nombre_base.".".$extension;?>" title="<?php echo $nombre_base.".".$extension;?>" />
  <!--<img  src="../../../subidos/clientepago_fotos/<?php echo $nombre_base."_thumb.".$extension;?>" title="<?php echo $nombre_base."_thumb.".$extension;?>" />-->
  <?php
		} else {
?>
...
}elseif(!empty($_SESSION['SesCpaFoto'.$Identificador])){
