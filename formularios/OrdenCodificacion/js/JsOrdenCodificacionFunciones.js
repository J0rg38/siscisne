...
"CmpOrdenCodificacionDetalleAno",
"CmpOrdenCodificacionDetalleModelo"

];

//"CmpOrdenCodificacionDetalleCodigo",
...
*/
	$("select#CmpMonedaId").change(function(){
		FncOrdenCodificacionEstablecerMoneda();
	});
...
function FncAlmacenMovimientoNavegar(oCampo){
		...
		if("CmpOrdenCodificacionDetalleModelo"==oCampo){
			$('#CmpOrdenCodificacionDetalleModelo').blur();
			FncOrdenCodificacionDetalleGuardar();
		}
...
function FncOrdenCodificacionEstablecerMoneda(){
	...
	if(MonedaId==""){
		$('#CmpTipoCambio').val('');
		FncOrdenCodificacionDetalleListar();
...
function FncImprmir(oId,oTalonario){
	FncPopUp('formularios/OrdenCodificacion/FrmOrdenCodificacionImprimir.php?Id='+oId+'&P=1',0,0,1,0,0,1,0,screen.height,screen.width);
}

function FncVistaPreliminar(oId,oTalonario){
	FncPopUp('formularios/OrdenCodificacion/FrmOrdenCodificacionImprimir.php?Id='+oId,0,0,1,0,0,1,0,screen.height,screen.width);
}
...
function FncProductoEscoger(oProId,oProNombre,oProPrecio,oProCosto,oProFoto,oProEspecificacion,oRtiId,oUmeId,oProCodigoOriginal,oProCodigoAlternativo,oUnidadMedidaIngreso,oProCostoIngreso,oProCostoIngresoNeto,oPcdId){	
	...
	$('#CmpOrdenCodificacionDetalleCodigo').val(oProCodigoOriginal);
	...
	$('#CmpOrdenCodificacionDetalleAno').focus();
