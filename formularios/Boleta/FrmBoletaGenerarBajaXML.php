...
$().ready(function() {
	...
	$.ajax({
		type: 'GET',
		dataType: 'json',
		url: 'acc/AccBoletaGenerarBajaXML.php',
		...
		success: function(respuesta){
			...
			switch(respuesta['CodigoRespuesta']){
				case "1":
					...
					if($GET_Procesar=="1"){
					?>	
						$('#CapBoletaAccion').html('Archivo xml generado correctamente, se enviara archivo al facturador...');
						window.setTimeout("FncBoletaEnviarBajaXML('"+respuesta['Nombre']+"');",2500);
...
function FncBoletaEnviarBajaXML(oNombre){
	
	FncPopUp('FrmBoletaEnviarBajaXML.php?Nombre='+oNombre+'&Id=<?php echo $InsBoleta->BolId;?>&Ta=<?php echo $InsBoleta->BtaId;?>&Procesar=<?php echo $GET_Procesar?>&EnviarSUNAT=<?php echo $GET_EnviarSUNAT;?>&P=1',0,0,1,0,0,1,0,350,150);
