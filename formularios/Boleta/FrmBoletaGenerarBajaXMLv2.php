...
$().ready(function() {
	...
	$.ajax({
		type: 'GET',
		dataType: 'json',
		url: 'acc/AccBoletaGenerarBajaXMLv2.php',
			...
			switch(respuesta['CodigoRespuesta']){
				case "1":
					...
					?>	
						...
						window.setTimeout("FncBoletaProcesarBajaXML('"+respuesta['Id']+"');",2500);
					<?php	
					}else{
					?>
						$('#CapBoletaAccion').html('Archivo xml de baja generado correctamente...');
...
function FncBoletaProcesarBajaXML(oId){
	
	//FncPopUp("FrmBoletaProcesarBajaXMLv2.php?Id="+oId+"&Procesar=<?php echo $GET_Procesar?>&EnviarSUNAT=<?php echo $GET_EnviarSUNAT;?>&P=1",0,0,1,0,0,1,0,350,150);
	window.location.href = 'FrmBoletaProcesarBajaXMLv2.php?Id='+oId+'&Procesar=<?php echo $GET_Procesar?>&EnviarSUNAT=<?php echo $GET_EnviarSUNAT;?>&P=1';
