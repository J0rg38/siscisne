...
function FncActualizarValidarStockSeleccionados(){
	...
	if(seleccionados==""){
		...
		var Tipo = prompt("Escoja una opcion \n 1 = Validar Stock  \n 2 = No Validar Stock", "1");
			
			if(Tipo !== null){
				switch(Tipo.toUpperCase()){
					case "1":
						document.getElementById('Acc').value= 'ActualizarAlmacenMovimientoEntradaValidarStockSi';
						$("#FrmListado").submit();
					break;
					
					case "2":
						document.getElementById('Acc').value= 'ActualizarAlmacenMovimientoEntradaValidarStockNo';
						$("#FrmListado").submit();
					break;
...
function FncNotaCreditoCompraListado(oAmoId){
	...
	tb_show(this.title,'formularios/AlmacenMovimientoEntrada/DiaNotaCreditoCompraListado.php?AmoId='+oAmoId,this.rel);		
