...
function FncActualizarEstadoRealizadoSeleccionados(){
	...
	if(seleccionados==""){
		...
		if(confirm("¿Realmente desea cambiar a estado REALIZADO los elementos?")){
			document.getElementById('Acc').value= 'ActualizarEstadoRealizado';
			$("#FrmListado").submit();
		}
...
function FncImprmir(oId){
	FncPopUp('formularios/VehiculoIngresoEvento/FrmVehiculoIngresoEventoImprimir.php?Id='+oId+'&P=1',0,0,1,0,0,1,0,screen.height,screen.width);
}

function FncVistaPreliminar(oId){
	FncPopUp('formularios/VehiculoIngresoEvento/FrmVehiculoIngresoEventoImprimir.php?Id='+oId,0,0,1,0,0,1,0,screen.height,screen.width);
}
