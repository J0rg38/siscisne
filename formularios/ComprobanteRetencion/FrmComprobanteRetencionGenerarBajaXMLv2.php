...
$().ready(function() {
	...
	$.ajax({
		type: 'GET',
		dataType: 'json',
		url: 'acc/AccComprobanteRetencionGenerarBajaXMLv2.php',
			...
			switch(respuesta['CodigoRespuesta']){
				case "1":
					...
					?>	
						...
						window.setTimeout("FncComprobanteRetencionProcesarBajaXML('"+respuesta['Id']+"');",2500);
					<?php	
					}else{
					?>
						$('#CapComprobanteRetencionAccion').html('Archivo xml de baja generado correctamente...');
...
function FncComprobanteRetencionProcesarBajaXML(oId){
	
	FncPopUp("FrmComprobanteRetencionProcesarBajaXMLv2.php?Id="+oId+"&Procesar=<?php echo $GET_Procesar?>&EnviarSUNAT=<?php echo $GET_EnviarSUNAT;?>&P=1",0,0,1,0,0,1,0,350,150);
