...
$().ready(function() {
	...
	$('#CapComprobanteRetencionAccion').html('Generando archivo xml...');	
	
	$.ajax({
		type: 'GET',
		dataType: 'json',
		url: 'acc/AccComprobanteRetencionGenerarXMLv2.php',
			...
			switch(respuesta['CodigoRespuesta']){
				case "1":
					...
					?>	
						...
						$('#CapComprobanteRetencionAccion').html('Archivo xml generado correctamente, se conectara con SUNAT...');
						window.setTimeout("FncComprobanteRetencionProcesarXML('"+respuesta['Nombre']+"');",2500);
						
					<?php	
					}else{
					?>
						
						$('#CapComprobanteRetencionAccion').html('Archivo xml generado correctamente...');
				...
				default:
					$('#CapComprobanteRetencionAccion').html('Ha ocurrido un error generando el archivo xml, proceso cancelado...');
...
function FncComprobanteRetencionProcesarXML(oNombre){
	
	FncPopUp('FrmComprobanteRetencionProcesarXMLv2.php?Nombre='+oNombre+'&Id=<?php echo $InsComprobanteRetencion->CrnId;?>&Ta=<?php echo $InsComprobanteRetencion->CrtId;?>&Procesar=<?php echo $GET_Procesar?>&EnviarSUNAT=<?php echo $GET_EnviarSUNAT;?>&P=1',0,0,1,0,0,1,0,350,150);
