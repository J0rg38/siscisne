...
//		FncVehiculoIngresoBuscar("Id");
	...
	//	
//		$("#CmpVehiculoIngresoId").val(InsCliente.EinId);	
//		$("#CmpTipoReferidoPresupuestoClienteTipo").val(InsCliente.LtiId);	
//		
//		//var VehiculoIngresoId = $("#CmpVehiculoIngresoId").val();		
...
//		}
		...
		FncTipoReferidoMantenimientoKilometrajeEstablecer();
...
function FncTipoReferidoMantenimientoKilometrajeEstablecer(){
	...
	$.getJSON("comunes/Vehiculo/JnPlanMantenimientoKilometraje.php?VehiculoMarcaId="+VehiculoMarcaId,{}, function(j){
		...
		$('select#CmpTipoReferidoPresupuestoMantenimientoKilometraje').html(options);
...
function FncTipoReferidoCalendarioCargarFormulario(oForm){
	
	
	//FncCargarVentanaNuevo(oRuta,oIframe,oModal,oTitulo)
	FncCargarVentanaNuevo('principal2.php?Mod=TipoReferido&Form='+oForm+'&Dia=1',"true","true","");
	
//	tb_show(this.title,'principal2.php?Mod=TipoReferido&Form='+oForm+'&Dia=1&placeValuesBeforeTB_=savedValues&TB_iframe=true&height=620&width=890&modal=true',this.rel);		
