	...
	var Tipo = prompt("Escoja el tipo de reporte \n 1 = Resumido\n 2 = Detallado", "1");
			...
			if(Tipo !== null){
				switch(Tipo.toUpperCase()){
					case "1":
	document.getElementById("FrmListado").action = "formularios/VehiculoMovimientoEntradaSimple/acc/AccVehiculoMovimientoEntradaSimpleGenerarExcel.php";
					break;
					
					case "2":
	document.getElementById("FrmListado").action = "formularios/VehiculoMovimientoEntradaSimple/acc/AccVehiculoMovimientoEntradaSimpleGenerarExcel2.php";
					break;
...
function FncImprmir(oId){
	FncPopUp('formularios/VehiculoMovimientoEntradaSimple/FrmVehiculoMovimientoEntradaSimpleImprimir.php?Id='+oId+'&P=1',0,0,1,0,0,1,0,screen.height,screen.width);
}

function FncVistaPreliminar(oId){
	FncPopUp('formularios/VehiculoMovimientoEntradaSimple/FrmVehiculoMovimientoEntradaSimpleImprimir.php?Id='+oId,0,0,1,0,0,1,0,screen.height,screen.width);
}
...
function FncNotaCreditoCompraListado(oCveId){
	...
	//FncPopUp('formularios/VehiculoMovimientoEntradaSimple/DiaNotaCreditoCompraListado.php?CveId='+oCveId,0,0,1,0,0,1,0,screen.height,screen.width);
	
	tb_show(this.title,'formularios/VehiculoMovimientoEntradaSimple/DiaNotaCreditoCompraListado.php?CveId='+oCveId,this.rel);		
...
function FncActualizarEstadoPendienteSeleccionados(){
	...
	if(seleccionados==""){
		...
		if(confirm("¿Realmente desea actualizar a No Realizado los elementos seleccionados?")){
...
function FncActualizarEstadoRealizadoSeleccionados(){
	...
	if(seleccionados==""){
		...
		if(confirm("¿Realmente desea actualizar a estado Realizado los elementos seleccionados?")){
...
//1,2,3,4,5
			...
			if(actualizar){
				$("#Acc").val("ActualizarEstadoRealizado");
				$("#FrmListado").submit();	
			}else{
				alert("Uno o mas de los elementos seleccionados no puede ser actualizado, verifique sus ESTADOS.");
			}
...
function FncActualizarEstadoAnuladoSeleccionados(){
	...
	if(seleccionados==""){
		...
		if(confirm("¿Realmente desea actualizar a estado Anulado los elementos seleccionados?")){
