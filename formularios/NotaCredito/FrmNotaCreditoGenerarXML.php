...
$().ready(function() {
	...
	$.ajax({
		type: 'GET',
		dataType: 'json',
		url: 'acc/AccNotaCreditoGenerarXML.php',
		data: 'Id=<?php echo $InsNotaCredito->NcrId;?>&Ta=<?php echo $InsNotaCredito->BtaId;?>',
		success: function(respuesta){
			...
			switch(respuesta['CodigoRespuesta']){
				case "1":
					...
					if($GET_Procesar=="1"){
					?>	
						$('#CapNotaCreditoAccion').html('Archivo xml generado correctamente, se enviara al facturador...');					
						window.setTimeout("FncNotaCreditoEnviarXML('"+respuesta['Nombre']+"');",2500);
					...
					//$('#CapNotaCreditoAccion').html('Archivo xml generado correctamente, se enviara archivo al facturador...');
					
					//FncPopUp('FrmNotaCreditoEnviarXML.php?Nombre='+respuesta['Nombre']+'&Id=<?php echo $InsNotaCredito->NcrId;?>&Ta=<?php echo $InsNotaCredito->BtaId;?>&Procesar=<?php echo $GET_Procesar?>&EnviarSUNAT=<?php echo $GET_EnviarSUNAT;?>&P=1',0,0,1,0,0,1,0,350,150);
...
function FncNotaCreditoEnviarXML(oNombre){
	
	FncPopUp('FrmNotaCreditoEnviarXML.php?Nombre='+oNombre+'&Id=<?php echo $InsNotaCredito->NcrId;?>&Ta=<?php echo $InsNotaCredito->BtaId;?>&Procesar=<?php echo $GET_Procesar?>&EnviarSUNAT=<?php echo $GET_EnviarSUNAT;?>&P=1',0,0,1,0,0,1,0,350,150);
