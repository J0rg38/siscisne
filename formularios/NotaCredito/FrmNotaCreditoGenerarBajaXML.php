...
$().ready(function() {
	...
	$.ajax({
		type: 'GET',
		dataType: 'json',
		url: 'acc/AccNotaCreditoGenerarBajaXML.php',
			...
			switch(respuesta['CodigoRespuesta']){
				case "1":
					...
					?>	
						...
						window.setTimeout("FncNotaCreditoEnviarBajaXML('"+respuesta['Nombre']+"');",2500);

					<?php	
					}else{
					?>
						$('#CapNotaCreditoAccion').html('Archivo xml de baja generado correctamente...');
...
function FncNotaCreditoEnviarBajaXML(oNombre){
	
	FncPopUp("FrmNotaCreditoEnviarBajaXML.php?Nombre="+oNombre+"&Id=<?php echo $InsNotaCredito->NcrId;?>&Ta=<?php echo $InsNotaCredito->NctId;?>&Procesar=<?php echo $GET_Procesar?>&EnviarSUNAT=<?php echo $GET_EnviarSUNAT;?>&P=1",0,0,1,0,0,1,0,350,150);
