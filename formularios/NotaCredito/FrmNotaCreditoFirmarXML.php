...
$().ready(function() {
	...
	$('#CapNotaCreditoAccion').html('Firmando archivo xml...');	
	...
	$.ajax({
		...
		complete: function(response) {
			...
			window.setTimeout("FncNotaCreditoFirmarXML();",5000);
...
function FncNotaCreditoFirmarXML(){
			...
			$.ajax({
				...
				data: 'hddNumRuc=<?php echo $EmpresaCodigo;?>&hddTipDoc=07&hddNumDoc=<?php echo $InsNotaCredito->NctNumero;?>-<?php echo $InsNotaCredito->NcrId;?>&hddNomArc=<?php echo $EmpresaCodigo;?>-07-<?php echo $InsNotaCredito->NctNumero;?>-<?php echo $InsNotaCredito->NcrId;?>&hddEstArc=07',
				
				complete: function(response) {
				
					//window.setTimeout("FncNotaCreditoRecibirFirmaXML();",5000);
					
								
					<?php
					if($GET_EnviarSUNAT==1 or $GET_EnviarSUNAT=="1"){
					?>
						$('#CapNotaCreditoAccion').html('Archivo xml firmado correctamente, se enviara a SUNAT...');					
						window.setTimeout("FncNotaCreditoRecibirFirmaXML();",5000);
						
					<?php	
					}else{
					?>	
						$('#CapNotaCreditoAccion').html('Archivo xml firmado correctamente...');	
...
function FncNotaCreditoRecibirFirmaXML(){
	
	FncPopUp('FrmNotaCreditoRecibirFirmaXML.php?Nombre=<?php echo $GET_Nombre;?>&Id=<?php echo $InsNotaCredito->NcrId;?>&Ta=<?php echo $InsNotaCredito->NctId;?>&Procesar=<?php echo $GET_Procesar;?>&EnviarSUNAT=<?php echo $GET_EnviarSUNAT;?>&P=1',0,0,1,0,0,1,0,350,150);
