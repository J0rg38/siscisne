...
$().ready(function() {
	...
	$.ajax({
		type: 'GET',
		dataType: 'json',
		url: 'acc/AccNotaCreditoGenerarBajaXMLv2.php',
			...
			switch(respuesta['CodigoRespuesta']){
				case "1":
					...
					?>	
						...
						window.setTimeout("FncNotaCreditoProcesarBajaXML('"+respuesta['Id']+"');",2500);
					<?php	
					}else{
					?>
...
function FncNotaCreditoProcesarBajaXML(oId){
	
	//FncPopUp("FrmNotaCreditoProcesarBajaXMLv2.php?Id="+oId+"&Procesar=<?php echo $GET_Procesar?>&EnviarSUNAT=<?php echo $GET_EnviarSUNAT;?>&P=1",0,0,1,0,0,1,0,350,150);
	window.location.href = 'FrmNotaCreditoProcesarBajaXMLv2.php?Id='+oId+'&Procesar=<?php echo $GET_Procesar?>&EnviarSUNAT=<?php echo $GET_EnviarSUNAT;?>&P=1';
