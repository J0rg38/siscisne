...
$InsNotaCredito->MtdGenerarNotaCreditoBajaId();

$NOMBRE = $EmpresaCodigo."-RA-".date("Ymd")."-".$InsNotaCredito->NcrSunatRespuestaBajaId;
$ARCHIVO = $NOMBRE.".xml";
$NOMBRE_INTERNO = "RA-".date("Ymd")."-".$InsNotaCredito->NcrSunatRespuestaBajaId;
...
//cbc:ReferenceDate
$ReferenceDate = $domtree->createElement("cbc:ReferenceDate",FncCambiaFechaAMysql($InsNotaCredito->NcrFechaEmision));
...
$AccountingSupplierParty = $xmlRoot->appendChild($AccountingSupplierParty);
	...
	$Party = $AccountingSupplierParty->appendChild($Party);
		...
		$VoidedDocumentsLine = $xmlRoot->appendChild($VoidedDocumentsLine);	
			...
			$DocumentTypeCode = $domtree->createElement("cbc:DocumentTypeCode","07");
			$DocumentTypeCode = $VoidedDocumentsLine->appendChild($DocumentTypeCode);	
			
			//cbc:DocumentSerialID
			$DocumentSerialID = $domtree->createElement("sac:DocumentSerialID",$InsNotaCredito->NctNumero);
			$DocumentSerialID = $VoidedDocumentsLine->appendChild($DocumentSerialID);	
			
			//cbc:DocumentNumberID
			$DocumentNumberID = $domtree->createElement("sac:DocumentNumberID",$InsNotaCredito->NcrId);
...
$InsNotaCredito->MtdEditarNotaCreditoDato("NcrSunatRespuestaBajaId",$InsNotaCredito->NcrSunatRespuestaBajaId,$InsNotaCredito->NcrId,$InsNotaCredito->NctId);
