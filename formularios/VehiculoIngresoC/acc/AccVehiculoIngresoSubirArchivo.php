...
<tr>
  ...
  <?php
	if($POST_Borrar=="1"){
	?>
    <?php $_SESSION['SesEinArchivoDAM'.$Identificador] = ""; ?>
    <?php	
	}
?>
    <form method="post" enctype="multipart/form-data">
...
if (!empty($_FILES)) {
	...
	if($extension=="jpg" || $extension == "png" || $extension == "gif" || $extension == "jpeg"){
		if (move_uploaded_file($tempFile,$targetFile)){
			...
			$_SESSION['SesEinArchivoDAM'.$Identificador] = $file_name;
...
}elseif(!empty($_SESSION['SesEinArchivoDAM'.$Identificador])){
	
	$extension = strtolower(pathinfo($_SESSION['SesEinArchivoDAM'.$Identificador], PATHINFO_EXTENSION));
	$nombre_base = basename($_SESSION['SesEinArchivoDAM'.$Identificador], '.'.$extension);  
?>
