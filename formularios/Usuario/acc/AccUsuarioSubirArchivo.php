...
if (!empty($_FILES)) {
	...
	$targetPath = '../../../subidos/usuario_fotos/';
	...
	if($extension=="jpg" || $extension == "png" || $extension == "gif" || $extension == "jpeg"){
		...
		if (move_uploaded_file($tempFile,$targetFile)){
			...
			$_SESSION['SesUsuFoto'] = $file_name;
			
			$mythumb = new thumb();
			$mythumb->loadImage('../../../subidos/usuario_fotos/'.$file_name);
			$mythumb->crop(101, 100,'bottom');
			$mythumb->save('../../../subidos/usuario_fotos/'.$nombre_base.'_thumb.'.$extension, 100);
			
			
			$mythumb = new thumb();
			$mythumb->loadImage('../../../subidos/usuario_fotos/'.$file_name);
			$mythumb->crop(21, 20,'bottom');
			$mythumb->save('../../../subidos/usuario_fotos/'.$nombre_base.'_thumb2.'.$extension, 20);
...
}elseif(!empty($_SESSION['SesUsuFoto'])){
