...
function FncValidar(){
	...
	var Total = $("#CmpTotal").val();
		...
		}else if(FncValidarPersonalHoraVehiculoIngresoBono()==false){
...
*/		
	...
	$('#BtnValidarVehiculoIngresoBono').on('click', function() {
		
		FncValidarPersonalHoraVehiculoIngresoBono();
		
	});
	
	
	
	$('#BtnVehiculoIngresoBonoCalendario').on('click', function() {

		FncVehiculoIngresoBonoCalendarioCargarFormulario();

	});
	
	$('#BtnVehiculoIngresoBonoVerRestricciones').on('click', function() {

		FncVehiculoIngresoBonoVerRestriccionesoCargarFormulario();
...
//	});
	...
	$("select#CmpMonedaId").change(function(){
		FncVehiculoIngresoBonoEstablecerMoneda();
	});
...
function FncVehiculoIngresoBonoMantenimientoKilometrajeEstablecer(){
	
	console.log("FncVehiculoIngresoBonoMantenimientoKilometrajeEstablecer");
...
console.log(VehiculoMarcaId);
	$.getJSON("comunes/Vehiculo/JnPlanMantenimientoKilometraje.php?VehiculoMarcaId="+VehiculoMarcaId,{}, function(j){
		...
		$('select#CmpVehiculoIngresoBonoPresupuestoMantenimientoKilometraje').html(options);
...
function FncVehiculoIngresoBonoCalendarioCargarFormulario(){
	
	//tb_show(this.title,'principal2.php?Mod=VehiculoIngresoBono&Form=VerCalendarioFull&Dia=1&placeValuesBeforeTB_=savedValues&TB_iframe=true&height=620&width=890&modal=true',this.rel);		
	FncCargarVentanaFullv3('principal2','VehiculoIngresoBono','VerCalendarioFull','');
...
function FncVehiculoIngresoBonoVerRestriccionesoCargarFormulario(){
	
	//tb_show(this.title,'principal2.php?Mod=VehiculoIngresoBono&Form=VerRestricciones&Dia=1&placeValuesBeforeTB_=savedValues&TB_iframe=true&height=280&width=480&modal=true',this.rel);
	
	FncCargarVentanaFullv3('principal2','VehiculoIngresoBono','VerRestricciones','');		

}

function FncValidarPersonalHoraVehiculoIngresoBono(){
	...
	/*	if(PersonalMecanico==""){
			...
			dhtmlx.alert({
				...
				$.ajax({
					type: 'POST',
					dataType: "json",
					url: 'formularios/VehiculoIngresoBono/acc/AccValidarVehiculoIngresoBono.php',
					data: 'FechaProgramada='+FechaProgramada+
					'&HoraProgramada='+(HoraProgramada)+
					'&Sucursal='+(Sucursal)+
					'&PersonalMecanico='+PersonalMecanico,
					success: function(InsVehiculoIngresoBono){
						
						if(InsVehiculoIngresoBono['respuesta']=="2"){

							dhtmlx.confirm("Se ha alcanzado el total de citas permitidos en este horario. ("+InsVehiculoIngresoBono['citas']+" de "+InsVehiculoIngresoBono['limite']+" citas). ¿Desea continuar de todas maneras?", function(result){
								if(result==true){		
									respuesta = true;
								}else{
									respuesta = false;
							...
							var saldo = InsVehiculoIngresoBono['limite'] - InsVehiculoIngresoBono['citas']

							dhtmlx.alert({
								title:"Aviso",
								type:"alert",
								text:"VehiculoIngresoBono disponible en este horario ("+saldo+" de "+InsVehiculoIngresoBono['limite']+" citas)",
								callback: function(result){
...
//							});
						...
						//if(InsVehiculoIngresoBono.CitId !=null){
//								
...
function FncCargarPersonalHorario(){
	...
	if(FechaProgramada != "" && HoraProgramada=="" && PersonalMecanico==""){
		...
		$.ajax({
			type: 'POST',
			dataType: "json",
			url: 'formularios/VehiculoIngresoBono/CapPersonalHorario.php',
			data: 'FechaProgramada='+FechaProgramada+
			'&HoraProgramada='+(HoraProgramada)+
			'&PersonalMecanico='+PersonalMecanico,
...
function FncVehiculoIngresoBonoHistorialListar(){
console.log("FncVehiculoIngresoBonoHistorialListar");
	var Identificador = $('#Identificador').val();

	var VehiculoIngresoVIN = $("#CmpVehiculoIngresoVIN").val();
	var VehiculoIngresoId = $("#CmpVehiculoIngresoId").val();	

	$('#CapVehiculoIngresoBonoHistorialAccion').html('Cargando...');
	
	$.ajax({
		type: 'POST',
		url: 'formularios/VehiculoIngresoBono/FrmVehiculoIngresoBonoHistorial.php',
		data: 'VehiculoIngresoVIN='+VehiculoIngresoVIN+'&VehiculoIngresoId='+VehiculoIngresoId,
		success: function(html){
			$('#CapVehiculoIngresoBonoHistorialAccion').html('Listo');	
			$("#CapVehiculoIngresoBonoHistoriales").html("");
			$("#CapVehiculoIngresoBonoHistoriales").append(html);
		}
	});
...
function FncVehiculoIngresoBonoEstablecerMoneda(){
