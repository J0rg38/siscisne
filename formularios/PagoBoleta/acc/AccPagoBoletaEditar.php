...
function FncCargarDatos(){
	...
	if($InsPago->MonId<>$EmpresaMonedaId){
		$InsPago->BolTotal = round($InsPago->BolTotal  / $InsPago->BolTipoCambio,2);
	}	
	...
	if(!empty($InsPago->PagoComprobante)){
		...
		foreach($InsPago->PagoComprobante as $DatPagoComprobante){
			...
			$BoletaId = $DatPagoComprobante->BolId;
			$BoletaTalonarioId = $DatPagoComprobante->BtaId;
			$BoletaTalonarioNumero = $DatPagoComprobante->BtaNumero;
