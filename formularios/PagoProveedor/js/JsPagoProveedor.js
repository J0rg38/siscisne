...
function FncGenerarExcel(){
	document.getElementById("FrmListado").action = "formularios/PagoProveedor/acc/AccPagoProveedorGenerarExcel.php"
...
function FncActualizarEstadoAnuladoSeleccionados(){
	...
	if(seleccionados==""){
		...
		if(confirm("¿Realmente desea ANULAR los pagos a proveedor seleccionadas?")){
...
function FncActualizarEstadoRealizadoSeleccionados(){
	...
	if(seleccionados==""){
		...
		if(confirm("¿Realmente desea POVANULAR los PAGO A PROVEEDOR seleccionadas?")){
...
//1,2,3,4,5
			...
			if(actualizar){
				$("#Acc").val("RealizadoPagoProveedor");
				$("#FrmListado").submit();	
			}else{
				alert("Uno o mas de los elementos seleccionados no puede ser actualizado, verifique sus ESTADOS.");
			}
...
function FncImprmir(oId){
	...
	FncPopUp('formularios/PagoProveedor/FrmPagoProveedorImprimir.php?Id='+oId+'&P=1',0,0,1,0,0,1,0,screen.height,screen.width);
...
function FncVistaPreliminar(oId){
	...
	FncPopUp('formularios/PagoProveedor/FrmPagoProveedorImprimir.php?Id='+oId,0,0,1,0,0,1,0,screen.height,screen.width);
