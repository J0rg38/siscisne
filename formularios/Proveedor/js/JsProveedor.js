	...
	if(seleccionados==""){
		...
		if((aux.length-1)>1){
			...
			$('input[type=checkbox]').each(function () {
				if($(this).attr('name')=="cmp_seleccionar[]"){
					if($(this).is(':checked')){
						window.location.href = "principal.php?Mod=Proveedor&Form=Editar&Id="+$(this).val();
					}
				}			 
			});
	...
	if(seleccionados==""){
		...
		if((aux.length-1)>1){
			...
			$('input[type=checkbox]').each(function () {
				if($(this).attr('name')=="cmp_seleccionar[]"){
					if($(this).is(':checked')){
						window.location.href = "principal.php?Mod=Proveedor&Form=Ver&Id="+$(this).val();
					}
				}			 
			});
...
function FncGenerarExcel(){
	document.getElementById("FrmListado").action = "formularios/Proveedor/acc/AccProveedorGenerarExcel.php"
	document.getElementById("FrmListado").submit();
	document.getElementById("FrmListado").action = "#";
}

function FncListadoImprimir(){
	document.getElementById("FrmListado").action = "formularios/Proveedor/FrmProveedorListadoImprimir.php"
