...
$pdf->Cell(100,5,'REPUESTOS',$Borde,0,'L');
 ...
 if($InsCotizacionProducto->CprDescuento>0){
				...
				foreach($InsCotizacionProducto->CotizacionProductoDetalle as $DatCotizacionProductoDetalle){
					...
					if( $InsCotizacionProducto->CprVerificar == 2 or ( $InsCotizacionProducto->CprVerificar == 1 and $DatCotizacionProductoDetalle->CrdEstado == 1)){
						...
						if($InsCotizacionProducto->MonId<>$EmpresaMonedaId){
								...
								$DatCotizacionProductoDetalle->CrdDescuento = ($DatCotizacionProductoDetalle->CrdDescuento / $InsCotizacionProducto->CprTipoCambio);
								$DatCotizacionProductoDetalle->CrdAdicional = ($DatCotizacionProductoDetalle->CrdAdicional / $InsCotizacionProducto->CprTipoCambio);
								
								$DatCotizacionProductoDetalle->CrdImporte = ($DatCotizacionProductoDetalle->CrdImporte / $InsCotizacionProducto->CprTipoCambio);
								$DatCotizacionProductoDetalle->CrdImporteBruto = ($DatCotizacionProductoDetalle->CrdImporteBruto / $InsCotizacionProducto->CprTipoCambio);
					...
					$pdf->Cell(75,5,utf8_decode($DatCotizacionProductoDetalle->ProNombre),$Borde,0,'L');
					$pdf->Cell(15,5,number_format($DatCotizacionProductoDetalle->CrdPrecioBruto,2),$Borde,0,'R');
					$pdf->Cell(15,5,number_format($DatCotizacionProductoDetalle->CrdImporteBruto,2),$Borde,0,'R');
		
					$pdf->Cell(15,5,number_format($DatCotizacionProductoDetalle->CrdDescuento,2),$Borde,0,'R');
					$pdf->Cell(15,5,number_format($DatCotizacionProductoDetalle->CrdImporte,2),$Borde,0,'R');
			
					
					
					$TotalRepuesto += $DatCotizacionProductoDetalle->CrdImporte;
					$TotalDescuento +=$DatCotizacionProductoDetalle->CrdDescuento;
		...
		if( ($InsCotizacionProducto->CprVerificar == 2 and $InsCotizacionProducto->CprRepuesto == "Si") or ($InsCotizacionProducto->CprVerificar == 1 and $InsCotizacionProducto->CprRepuestoVerificado== "Si" ) ){
			...
			if(!empty($InsCotizacionProducto->CotizacionProductoDetalle)){
				foreach($InsCotizacionProducto->CotizacionProductoDetalle as $DatCotizacionProductoDetalle){
					...
					$pdf->Cell(15,5,number_format($DatCotizacionProductoDetalle->CrdPrecio ,2),$Borde,0,'R');
					$pdf->Cell(15,5,number_format($DatCotizacionProductoDetalle->CrdImporte,2),$Borde,0,'R');
...
//$InsCotizacionProducto->CprObservacionImpresa =str_replace("</p>",chr(13),$InsCotizacionProducto->CprObservacionImpresa);
$InsCotizacionProducto->CprObservacionImpresa = strip_tags($InsCotizacionProducto->CprObservacionImpresa,"");
//$InsCotizacionProducto->CprObservacionImpresa =str_replace("\n",' ',$InsCotizacionProducto->CprObservacionImpresa);
		
if(!empty($InsCotizacionProducto->CprObservacionImpresa)){

	$Observacion = "";

	if(strlen($InsCotizacionProducto->CprObservacionImpresa)>70){

		$vueltas = round(strlen($InsCotizacionProducto->CprObservacionImpresa)/70)+1;

		$primera = 0;
		for($i=0;$i<$vueltas;$i++){

			$Observacion = substr($InsCotizacionProducto->CprObservacionImpresa,$primera,70);
			$pdf->SetFont('Courier','B',8);
		...
		$pdf->SetFont('Courier','',10);
		$pdf->Cell(100,5,utf8_decode($InsCotizacionProducto->CprObservacionImpresa),$Borde,0,'L');
		$InsCotizacionProducto->CprObservacionImpresa=str_replace("\n",' ',$InsCotizacionProducto->CprObservacionImpresa);
...
$pdf->Cell(100,5,'- Todo pedido es con un 50% de adelanto.' ,$Borde,0,'L'); 
$pdf->Cell(70,5,'' ,$Borde,1,'L');            
...
$pdf->Cell(100,5,'- En caso desista de la compra, se retendra el 50% del valor total por gastos operativos generados.' ,$Borde,0,'L'); 
$pdf->Cell(70,5,'' ,$Borde,1,'L');

$pdf->Cell(100,5,'- Para importaciones la demora es de 90 dias a mas, esto sujeto a disposicion de General Motors Peru S.A.' ,$Borde,0,'L'); 
$pdf->Cell(70,5,'' ,$Borde,1,'L');   

$pdf->Cell(100,5,'- Los pedidos a Lima demoran 7 dias habiles como minimo, esto sujeto a disposicion de General Motors Peru S.A.' ,$Borde,0,'L'); 
$pdf->Cell(70,5,'' ,$Borde,1,'L');   
...
$pdf->Cell(90,5,'' ,$Borde,0,'L'); 	
$pdf->Cell(100,5,'Jefe de Repuestos',$Borde,1,'C'); 	
