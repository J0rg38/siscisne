...
if (isset($_POST['BtnGuardar_x']) or (isset($_POST['Guardar']) && $_POST['Guardar'] == "1")) {
	...
	$InsFichaIngreso->FinMantenimientoKilometraje = empty($_POST['CmpFichaIngresoMantenimientoKilometraje']) ? 0 : $_POST['CmpFichaIngresoMantenimientoKilometraje'];
	...
	//	$InsFichaIngreso->FichaIngresoEnviar = ($_POST['CmpFichaIngresoEnviar']);
	...
	if ($InsFichaIngreso->FinTipo == 2) {
		...
		if (!empty($ArrSesionObjetos)) {
			foreach ($ArrSesionObjetos as $DatSesionObjeto) {
				...
				$_SESSION['InsPreEntregaDetalle' . $Identificador]->MtdEditarSesionObjeto(
					$DatSesionObjeto->Item,
					1,
			...
			foreach ($ArrPreEntregaSecciones as $DatPreEntregaSeccion) {
				...
				foreach ($ArrPreEntregaTareas as $DatPreEntregaTarea) {
					...
					$_SESSION['InsPreEntregaDetalle' . $Identificador]->MtdAgregarSesionObjeto(
						1,
						$InsPreEntregaDetalle1->RedId,
						NULL,
						$InsPreEntregaDetalle1->PetId,
	...
	if (!empty($ArrSesionObjetos)) {
		foreach ($ArrSesionObjetos as $DatSesionObjeto) {
			...
			$InsFichaIngresoHerramienta1->FapId = $DatSesionObjeto->Parametro1;
			$InsFichaIngresoHerramienta1->ProId = $DatSesionObjeto->Parametro2;

			$InsFichaIngresoHerramienta1->UmeId = $DatSesionObjeto->Parametro6;
			$InsFichaIngresoHerramienta1->FapCantidad = $DatSesionObjeto->Parametro9;
			$InsFichaIngresoHerramienta1->FapCantidadReal = $DatSesionObjeto->Parametro10;

			$InsFichaIngresoHerramienta1->FapEstado = $DatSesionObjeto->Parametro14;
			$InsFichaIngresoHerramienta1->FapTiempoCreacion = FncCambiaFechaAMysql($DatSesionObjeto->Parametro7);
			$InsFichaIngresoHerramienta1->FapTiempoModificacion = FncCambiaFechaAMysql($DatSesionObjeto->Parametro8);
			$InsFichaIngresoHerramienta1->FapEliminado = 1;
			...
			$_SESSION['InsFichaAccionHerramienta' . $Identificador]->MtdEditarSesionObjeto(
				$DatSesionObjeto->Item,
				1,
				$InsFichaIngresoHerramienta1->FapId,
				$InsFichaIngresoHerramienta1->ProId,
				$InsFichaIngresoHerramienta1->ProNombre,
				NULL,
				NULL,
				$InsFichaIngresoHerramienta1->UmeId,
				FncCambiaFechaANormal($InsFichaIngresoHerramienta1->FapTiempoCreacion),
				FncCambiaFechaANormal($InsFichaIngresoHerramienta1->FapTiempoModificacion),
				$InsFichaIngresoHerramienta1->FapCantidad,
				$InsFichaIngresoHerramienta1->FapCantidadReal,
				$InsFichaIngresoHerramienta1->RtiId,
				$InsFichaIngresoHerramienta1->UmeNombre,
				$InsFichaIngresoHerramienta1->UmeIdOrigen,
				$InsFichaIngresoHerramienta1->FapEstado
			);
		}
	}
			...
			if (!empty($DatFichaIngresoModalidad->MinSigla)) { //AUX
				...
				if (!empty($ArrSesionObjetos)) {
					foreach ($ArrSesionObjetos as $DatSesionObjeto) {
						...
						$_SESSION['InsFichaAccionTarea' . $DatFichaIngresoModalidad->MinSigla . $Identificador]->MtdEditarSesionObjeto(
							$DatSesionObjeto->Item,
							$InsFichaAccionTarea1->FatEliminado,
				...
				if (!empty($ArrSesionObjetos)) {
					foreach ($ArrSesionObjetos as $DatSesionObjeto) {
						...
						$_SESSION['InsFichaAccionTempario' . $DatFichaIngresoModalidad->MinSigla . $Identificador]->MtdEditarSesionObjeto(
							$DatSesionObjeto->Item,
							$InsFichaAccionTempario1->FaeEliminado,
				...
				if (!empty($ArrSesionObjetos)) {
					foreach ($ArrSesionObjetos as $DatSesionObjeto) {
						...
						$_SESSION['InsFichaAccionProducto' . $DatFichaIngresoModalidad->MinSigla . $Identificador]->MtdEditarSesionObjeto(
							$DatSesionObjeto->Item,
							1,
				...
				if (!empty($ArrSesionObjetos)) {
					foreach ($ArrSesionObjetos as $DatSesionObjeto) {
						...
						$InsFichaAccionSuministro1->ProCodigoOriginal = $DatSesionObjeto->Parametro26;
						...
						$_SESSION['InsFichaAccionSuministro' . $DatFichaIngresoModalidad->MinSigla . $Identificador]->MtdEditarSesionObjeto(
							$DatSesionObjeto->Item,
							1,
						...
						foreach ($ArrPlanMantenimientoTareas as $DatPlanMantenimientoTarea) {
							...
							if (!empty($_POST['CmpPlanMantenimientoDetalleAccion_' . $DatPlanMantenimientoTarea->PmtId])) {
								...
								if (!empty($ArrSesionObjetos)) {
									foreach ($ArrSesionObjetos as $DatSesionObjeto) {
										...
										if ($DatSesionObjeto->Parametro3 == $DatPlanMantenimientoTarea->PmtId) {
											...
											$_SESSION['InsFichaAccionMantenimiento' . $DatFichaIngresoModalidad->MinSigla . $Identificador]->MtdEditarSesionObjeto(
												$DatSesionObjeto->Item,
												1,
								...
								if (!$ExistePlanMantenimientoDetalle) {
									...
									$_SESSION['InsFichaAccionMantenimiento' . $InsFichaAccion->MinSigla . $Identificador]->MtdAgregarSesionObjeto(
										1,
										$InsFichaAccionMantenimiento1->FaaId,
										NULL,
										$InsFichaAccionMantenimiento1->PmtId,
					...
					if ($InsFichaAccion->MinSigla <> "PP") {
						...
						if ($InsFichaAccion->MtdEditarFichaAccion()) {
							...
							if (!empty($InsFichaAccion->FccPedido)) {
								if (($InsFichaAccion->FccPedido) <> $InsFichaAccion->FccPedido2) {
	...
	$_SESSION['SesFinFotoVIN' . $Identificador] = $InsVehiculoIngreso->EinFotoVIN ?? '';
	$_SESSION['SesFinFotoFrontal' . $Identificador] = $InsVehiculoIngreso->EinFotoFrontal ?? '';
	$_SESSION['SesFinFotoCupon' . $Identificador] = $InsVehiculoIngreso->EinFotoCupon ?? '';
	...
	if ($InsFichaIngreso->FinEstado == 11) {
		...
		if (!empty($InsFichaIngreso->FichaIngresoModalidad)) {
			foreach ($InsFichaIngreso->FichaIngresoModalidad as $DatFichaIngresoModalidad) {
				...
				//				deb($DatFichaIngresoModalidad->FichaIngresoProducto);
					...
					foreach ($DatFichaIngresoModalidad->FichaIngresoProducto as $DatFichaIngresoProducto) {
						...
						if (!empty($DatFichaIngresoProducto->MinSigla)) { //AUX
							...
							if (!empty($DatFichaIngresoProducto->ProId)) {
								...
								$InsFichaAccionProducto1->UmeId = $DatFichaIngresoProducto->UmeId;
								$InsFichaAccionProducto1->FapAccion = "C";

								$InsFichaAccionProducto1->FapVerificar1 = 2; //SECAMBIO PRO DEFECTO 
								$InsFichaAccionProducto1->FapVerificar2 = 1;
								$InsFichaAccionProducto1->FapCantidad = $DatFichaIngresoProducto->FipCantidad;
					...
					foreach ($DatFichaIngresoModalidad->FichaIngresoSuministro as $DatFichaIngresoSuministro) {
						...
						if (!empty($DatFichaIngresoSuministro->MinSigla)) { //AUX
							...
							if (!empty($DatFichaIngresoSuministro->ProId)) {
								...
								$InsFichaAccionSuministro1->FasVerificar1 = 2; //SE CAMBI POR DEFECTO
								$InsFichaAccionSuministro1->FasVerificar2 = 2;
				...
				if ($DatFichaIngresoModalidad->MinId == "MIN-10001") {
					...
					if (!empty($DatFichaIngresoModalidad->FichaIngresoMantenimiento)) {
						...
						//$DatFichaIngresoModalidad->FichaIngresoMantenimiento	
				...
				if (empty($FichaAccionId)) {
					...
					if ($InsFichaAccion->MtdRegistrarFichaAccion()) {
						$validar++;
						FncFichaAccionCargarDatos(true);
					}
				} else {
					$validar++;
				}
...
function FncFichaAccionCargarDatos($oLlenarTareaProducto = false)
{
	...
	if (!empty($InsFichaAccion->FichaAccionTarea)) {
		foreach ($InsFichaAccion->FichaAccionTarea as $DatFichaAccionTarea) {
			...
			$_SESSION['InsFichaAccionTarea' . $InsFichaAccion->MinSigla . $Identificador]->MtdAgregarSesionObjeto(
				1,
	...
	if (!empty($InsFichaAccion->FichaAccionProducto)) {
		foreach ($InsFichaAccion->FichaAccionProducto as $DatFichaAccionProducto) {
			...
			$_SESSION['InsFichaAccionProducto' . $InsFichaAccion->MinSigla . $Identificador]->MtdAgregarSesionObjeto(
				1,
	...
	Parametro25 = FiaId
	Parametro26 = ProCodigoOriginal
	Parametro27 = ProIdFichaIngresoMantenimiento
	*/
	...
	if (!empty($InsFichaAccion->FichaAccionSuministro)) {
		foreach ($InsFichaAccion->FichaAccionSuministro as $DatFichaAccionSuministro) {
			...
			$_SESSION['InsFichaAccionSuministro' . $InsFichaAccion->MinSigla . $Identificador]->MtdAgregarSesionObjeto(
				1,
	...
	if (!empty($InsFichaAccion->FichaAccionTempario)) {
		foreach ($InsFichaAccion->FichaAccionTempario as $DatFichaAccionTempario) {
			...
			$_SESSION['InsFichaAccionTempario' . $InsFichaAccion->MinSigla . $Identificador]->MtdAgregarSesionObjeto(
				1,
