...
function FncValidar(){
		...
		var ProveedorNombreCompleto = $("#CmpProveedorNombreCompleto").val();
		...
		if(ProveedorId == ""){		
				...
				dhtmlx.alert({
					title:"Aviso",
					type:"alert-error",
					text:"Debe de escoger un proveedor",
					callback: function(result){
						$("#CmpProveedorNombreCompleto").focus();
					}
				});
		...
		}else if(ProveedorNombreCompleto == ""){			
...
function FncImprmir(oId,oTalonario){
	FncPopUp('formularios/PedidoCompraSimple/FrmPedidoCompraSimpleImprimir.php?Id='+oId+'&P=1',0,0,1,0,0,1,0,screen.height,screen.width);
}

function FncVistaPreliminar(oId,oTalonario){
	FncPopUp('formularios/PedidoCompraSimple/FrmPedidoCompraSimpleImprimir.php?Id='+oId,0,0,1,0,0,1,0,screen.height,screen.width);
}
