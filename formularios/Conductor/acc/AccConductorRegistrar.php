...
if(isset($_POST['BtnGuardar_x']) or $_POST['Guardar']=="1"){	
	...
	$InsConductor->ConVehiculoUnidad = $_POST['CmpVehiculoId'];		
...
//		
//		$InsConductorActividad->CacFecha = $_POST['CmpFechaInicio'];
//		$InsConductorActividad->CacTipo = 1;
//		$InsConductorActividad->CacTiempoCreacion = date("Y-m-d H:i:s");
//		$InsConductorActividad->CacTiempoModificacion = date("Y-m-d H:i:s");
//		
//	}

	if(!empty($InsConductor->ConNumeroDocumento)){
		if($InsConductor->MtdVerificarExisteConductorDato("ConNumeroDocumento",$InsConductor->ConNumeroDocumento,NULL)){
			$Resultado.='#ERR_CON_203';	
			$Guardar = false;
...
//				$///InsVehiculo->MtdEditarVehiculoDato("VehRevisionTecnicaFecha",$InsConductor->VehRevisionTecnicaFecha,$InsConductor->VehId);
			...
			$Registro = true;
			$Resultado.='#SAS_CON_101';	
			unset($_SESSION['SesConFoto'.$Identificador]);
			unset($InsConductor);
			
		} else{	
		
			$Resultado.='#ERR_CON_101';	
	...
	$InsConductor->ConFechaInicio = date("d/m/Y");
	
	$InsConductor->ConSupervisor = 2;
	$InsConductor->ConSupervisorNivel = 1;
