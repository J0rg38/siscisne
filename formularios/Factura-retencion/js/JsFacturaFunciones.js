...
//			alert("Debes ingresar una fecha de termino");			
		...
		}else if(ClienteTipoDocumento != "TDO-10003"){
		
				dhtmlx.alert({
					title:"Aviso",
					type:"alert-error",
					text:"Debes escoger solo como tipo de documento RUC",
					callback: function(result){
						$("#CmpClienteNumeroDocumento").focus();
					}
				});
...
$().ready(function() {
	...
	$('#CmpFechaEmision').on('keyup', function() {
		FncFacturaCalcularFechaVencimiento();
	});
...
function FncFacturaCalcularFechaVencimiento(){
	
	console.log("FncFacturaCalcularFechaVencimiento");
