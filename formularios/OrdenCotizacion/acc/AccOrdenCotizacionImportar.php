		...
		switch($ext){
			...
			case "xlsx":
				...
				foreach( $xlsx->rows() as $r ) { 
					...
					$OrdenCotizacionId  =  trim(( (!empty($r[$columna])) ? $r[$columna] : '' ));	

					if (strpos($OrdenCotizacionId, 'CCO') !== false) {
						break;
					}

				}
				
				echo "Numero de solicitud de cotizacion: ".$OrdenCotizacionId;
				echo "<br>";
...
//					echo "<br>";
					...
					$InsOrdenCotizacion->OotId = $OrdenCotizacionId;
					$InsOrdenCotizacion->MtdObtenerOrdenCotizacion();
					
					$InsOrdenCotizacion->MtdEditarOrdenCotizacionDato("OotCodigoReferencia",$CodigoReferencia,$InsOrdenCotizacion->OotId);
					

					if(!empty($InsOrdenCotizacion->OotEstado)){
						...
						foreach( $xlsx->rows() as $r ) { 
							...
							for( $columna=0; $columna < $num_cols; $columna++ ) {
								...
								if($columna == $inicio_columnaB){
									$OrdenCotizacionDetallePrecio  =  trim(( (!empty($r[$columna])) ? $r[$columna] : '' ));	
								}	
							...
							if(!empty($ProductoCodigoOriginal)){
								...
								if(!empty($OrdenCotizacionDetallePrecio)){
									
									echo "Codigo Original: ".$ProductoCodigoOriginal;
									echo "<br>";
									echo "Precio: ".$OrdenCotizacionDetallePrecio;
									echo "<br>";
									...
									$ResOrdenCotizacionDetalle = $InsOrdenCotizacionDetalle->MtdObtenerOrdenCotizacionDetalles("pro.ProCodigoOriginal",$ProductoCodigoOriginal,'OodId','Desc','1',$OrdenCotizacionId,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,"OotFecha",false,false,NULL);
									$ArrOrdenCotizacionDetalles = $ResOrdenCotizacionDetalle['Datos'];						
									
									$OrdenCotizacionDetalleId = "";
									
									if(!empty($ArrOrdenCotizacionDetalles)){
										foreach($ArrOrdenCotizacionDetalles as $DatOrdenCotizacionDetalle){
											$OrdenCotizacionDetalleId = $DatOrdenCotizacionDetalle->OodId;		
										}
									}
								
									if(!empty($OrdenCotizacionDetalleId)){
										
										if($InsOrdenCotizacion->MonId<> $EmpresaMonedaId){
											$PrecioConvertido = $OrdenCotizacionDetallePrecio * $InsOrdenCotizacion->OotTipoCambio;	
										}else{
											$PrecioConvertido = $OrdenCotizacionDetallePrecio;				
										}
				
										if($InsOrdenCotizacionDetalle->MtdEditarOrdenCotizacionDetalleDato("OodPrecio",$PrecioConvertido,$OrdenCotizacionDetalleId)){
											if($InsOrdenCotizacionDetalle->MtdEditarOrdenCotizacionDetalleDato("OodImporte",$PrecioConvertido,$OrdenCotizacionDetalleId)){
										...
										echo "Repuesto no encontrado en la solicitud de cotizacion";	
										echo "<br>";
									}
