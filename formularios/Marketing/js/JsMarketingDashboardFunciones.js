...
$().ready(function() {
	...
	$('#BtnExcel').on('click', function() {
		if(FncValidar()){
			FncMarketingDashboardGenerarExcel('');
		}
	});
...
function FncMarketingDashboardImprimir(oIndice){
	
	
	FncObtenerVariables();
	
	var Accion = document.getElementById('FrmMarketingDashboard'+oIndice).action;
	
	FncPopUp(Accion+"?"+Variables+"&P=1",0,0,1,0,0,1,0,screen.height,screen.width);
	
}

function FncMarketingDashboardGenerarExcel(oIndice){
...
function FncMarketingDashboardVer(oIndice){
	
	document.getElementById('FrmMarketingDashboard'+oIndice).submit();
