...
$ResReporteProductoVentasPerdida = $InsReporteProducto->MtdObtenerReporteProductoVentasPerdidas(NULL,$FechaInicio,$FechaFin,NULL,NULL,"cpr.CprFecha","DESC","25",$POST_VehiculoMarca,$POST_Sucursal);
$ArrReporteProductoVentasPerdidas = $ResReporteProductoVentasPerdida['Datos'];
 ...
 // Create new PHPExcel object
  ...
  $objReader = PHPExcel_IOFactory::createReader('Excel5');
			  ...
			  $objPHPExcel = $objReader->load("../../plantilla/TemCORChevrolet.xls");
...
$objPHPExcel->setActiveSheetIndex(0)
	...
	for($mes=1;$mes<=$POST_Mes;$mes++){
		...
		$ResReporteCOR = $InsReporteCOR->MtdObtenerReporteCORs(NULL,NULL,NULL,'RcrId','Desc','1',$POST_Ano,str_pad($mes,2,"0",STR_PAD_LEFT),$POST_VehiculoMarca,$POST_Sucursal);
	...
	//RcrVentaTallerMarca
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
				...
				$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'16', !empty($RcrStockMarca[$mes])?round($RcrStockMarca[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
				...
				$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'17', !empty($RcrStockLubricantes[$mes])?round($RcrStockLubricantes[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
				...
				$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'18', !empty($RcrTotalStock[$mes])?round($RcrTotalStock[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
				...
				$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'19', !empty($RcrValorRepuestosA[$mes])?round($RcrValorRepuestosA[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
					...
					$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'20', !empty($RcrValorRepuestosB[$mes])?round($RcrValorRepuestosB[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
				...
				$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'21', !empty($RcrValorRepuestosC[$mes])?round($RcrValorRepuestosC[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
				...
				$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'22', !empty($RcrValorRepuestosD[$mes])?round($RcrValorRepuestosD[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
				 ...
				 $objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'23', !empty($RcrRotationMarca[$mes])?round($RcrRotationMarca[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
                 ...
                 $objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'24', !empty($RcrValorPreObsoletos[$mes])?round($RcrValorPreObsoletos[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
				 ...
				 $objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'25', !empty($RcrValorObsoletos[$mes])?round($RcrValorObsoletos[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
				 ...
				 $objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'28', !empty($RcrPedidosYSTK[$mes])?round($RcrPedidosYSTK[$mes],2):'');
	...
	$Columna = 5;
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
			 	...
			 	$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'29', !empty($RcrPedidosYRUSH[$mes])?round($RcrPedidosYRUSH[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
				...
				$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'30', !empty($RcrPedidosZVOR[$mes])?round($RcrPedidosZVOR[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
				...
				$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'31', !empty($RcrPedidosZGAR[$mes])?round($RcrPedidosZGAR[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
				...
				$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'32', !empty($RcrTasaServicioTaller[$mes])?round($RcrTasaServicioTaller[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
				...
				$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'33', !empty($RcrMontoVentaPedidas[$mes])?round($RcrMontoVentaPedidas[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
				...
				$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'37', !empty($RcrPersonalAsesorRepuestos[$mes])?round($RcrPersonalAsesorRepuestos[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'38', !empty($RcrPersonalAsistenteAlmacen[$mes])?round($RcrPersonalAsistenteAlmacen[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			if( ($mes<=date("m") and date("Y")== $POST_Ano) or  $POST_Ano!=date("Y")){
				...
				$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'39', !empty($RcrDiasLaborados[$mes])?round($RcrDiasLaborados[$mes],2):'');
		...
		for($mes=1;$mes<=$POST_Mes;$mes++){
			...
			$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue(FncConvertirNumeroALetraExcel($Columna).'40', !empty($RcrHorasDisponibles[$mes])?round($RcrHorasDisponibles[$mes],2):'');
	...
	$objPHPExcel->setActiveSheetIndex(1)
				//->setCellValue('L42', $InsPersonal->PerNombre.' '.$InsPersonal->PerApellidoPaterno.' '.$InsPersonal->PerApellidoMaterno);	
				->setCellValue('L42', "");	
