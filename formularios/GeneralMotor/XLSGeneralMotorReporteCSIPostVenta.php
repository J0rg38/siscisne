...
$ArrReporteFichaIngresos = $ResReporteFichaIngreso['Datos'];
 ...
 // Create new PHPExcel object
  ...
  $objReader = PHPExcel_IOFactory::createReader('Excel5');
			  ...
			  $objPHPExcel = $objReader->load("../../plantilla/TemCSIPostVenta2.xls");
		...
		if(!empty($ArrReporteFichaIngresos)){
			foreach($ArrReporteFichaIngresos as $DatReporteFichaIngreso){
				...
				$objPHPExcel->setActiveSheetIndex(0)
								...
								$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue(FncConvertirNumeroALetraExcel(7).$fila, $DatReporteFichaIngreso->FinTiempoTrabajoTerminado);
...
if(!empty($DatReporteFichaIngreso->CliCelular)){
	...
	$Celular .= $DatReporteFichaIngreso->CliContactoCelular3;
								...
								$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue(FncConvertirNumeroALetraExcel(22).$fila, ( empty($DatReporteFichaIngreso->CliDepartamento)?$DatReporteFichaIngreso->SucDepartamento:$DatReporteFichaIngreso->CliDepartamento));
								
							
								$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue(FncConvertirNumeroALetraExcel(23).$fila, empty($DatReporteFichaIngreso->CliProvincia)?$DatReporteFichaIngreso->SucDepartamento:$DatReporteFichaIngreso->CliProvincia);
								
								$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue(FncConvertirNumeroALetraExcel(24).$fila, $DatReporteFichaIngreso->PerNumeroDocumento);
																
								$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue(FncConvertirNumeroALetraExcel(25).$fila, $DatReporteFichaIngreso->PerNombre);
								
								$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue(FncConvertirNumeroALetraExcel(26).$fila, $DatReporteFichaIngreso->PerApellidoPaterno." ".$DatReporteFichaIngreso->PerApellidoMaterno);
								
								
								$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue(FncConvertirNumeroALetraExcel(28).$fila, $DatReporteFichaIngreso->FinVehiculoKilometraje);
