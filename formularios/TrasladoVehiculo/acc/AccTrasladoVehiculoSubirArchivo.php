...
<tr>
  ...
  <?php
	if($POST_Borrar=="1"){
	?>
    <?php $_SESSION['SesTveFoto'.$Identificador] = ""; ?>
    <?php	
	}
?>
...
if (!empty($_FILES)) {
	...
	if($extension=="jpg" || $extension == "png" || $extension == "gif" || $extension == "jpeg"  || $extension == "pdf"){
		...
		if (move_uploaded_file($tempFile,$targetFile)){
			...
			$_SESSION['SesTveFoto'.$Identificador] = $file_name;
...
}elseif(!empty($_SESSION['SesTveFoto'.$Identificador])){
	
	$extension = strtolower(pathinfo($_SESSION['SesTveFoto'.$Identificador], PATHINFO_EXTENSION));
	$nombre_base = basename($_SESSION['SesTveFoto'.$Identificador], '.'.$extension);  
?>
