	...
	var Tipo = prompt("Escoja el tipo de reporte \n 1 = Resumido\n 2 = Detallado", "1");
			...
			if(Tipo !== null){
				switch(Tipo.toUpperCase()){
					case "1":
	document.getElementById("FrmListado").action = "formularios/VehiculoMovimientoSalidaSimple/acc/AccVehiculoMovimientoSalidaSimpleGenerarExcel.php";
					break;
					
					case "2":
	document.getElementById("FrmListado").action = "formularios/VehiculoMovimientoSalidaSimple/acc/AccVehiculoMovimientoSalidaSimpleGenerarExcel2.php";
					break;
...
function FncVehiculoMovimientoSalidaSimpleActualizarRealizadoSeleccionados(){
...
*/
	...
	if(seleccionados==""){
		...
		if(confirm("¿Realmente deseas marcar a Realizado los elementos seleccionados?")){
			...
			if(actualizar){
				$("#Acc").val("VehiculoMovimientoSalidaSimpleActualizarRealizado");
				$("#FrmListado").submit();	
			}else{
...
function FncVehiculoMovimientoSalidaSimpleActualizarPendienteSeleccionados(){
...
*/
	...
	if(seleccionados==""){
		...
		if(confirm("¿Realmente desea marcar como En Transito los elementos seleccionados?")){
		...
		/*	$('input[type=checkbox]').each(function () {
			...
			if(actualizar){	
				...
				$("#Acc").val("VehiculoMovimientoSalidaSimpleActualizarPendiente");
				$("#FrmListado").submit();
...
function FncVehiculoMovimientoSalidaSimpleActualizarAnuladoSeleccionados(){
...
*/
	...
	if(seleccionados==""){
		...
		if(confirm("¿Realmente desea marcar como Anulado los elementos seleccionados?")){
				...
				if($(this).attr('name')=="cmp_seleccionar[]"){
					...
					if(actualizar){	
						...
						if(actualizar2){	
							...
							$("#Acc").val("VehiculoMovimientoSalidaSimpleActualizarAnulado");
							$("#FrmListado").submit();
...
function FncImprmir(oId){
	FncPopUp('formularios/VehiculoMovimientoSalidaSimple/FrmVehiculoMovimientoSalidaSimpleImprimir.php?Id='+oId+'&P=1',0,0,1,0,0,1,0,screen.height,screen.width);
}
...
function FncNotaCreditoCompraListado(oCveId){
	
	//FncPopUp('formularios/VehiculoMovimientoSalidaSimple/DiaNotaCreditoCompraListado.php?CveId='+oCveId,0,0,1,0,0,1,0,screen.height,screen.width);
	
	tb_show(this.title,'formularios/VehiculoMovimientoSalidaSimple/DiaNotaCreditoCompraListado.php?CveId='+oCveId,this.rel);		
