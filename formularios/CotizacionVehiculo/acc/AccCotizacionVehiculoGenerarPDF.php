...
$InsPersonal = new ClsPersonal();
	...
	*/
	...
	$ArchivoFacturaCodigoQR = $InsPersonal->PerId.'026.png'; 
	$RutaFacturaCodigoQR = "../../../generados/".$ArchivoFacturaCodigoQR; 
	// generating 
	
	if (file_exists($RutaFacturaCodigoQR)) { 
		unlink($RutaFacturaCodigoQR); 
	}
	QRcode::png($codeContents, $RutaFacturaCodigoQR, QR_ECLEVEL_L, 3);
...
$pdf->SetTitle($InsCotizacionVehiculo->CveId);
...
if(!empty($InsCotizacionVehiculo->CveColor)){
	$Anos .= '- Color '.$InsCotizacionVehiculo->CveColor;	
}
...
$pdf->SetFont('times','B',10);
$pdf->Cell(70,5,('ASESOR DE VENTAS'),1,0,'L');



	
			
		$pdf->Image($RutaFacturaCodigoQR,180,55,NULL,25);
...
//	);
//	
//	$pdf->write2DBarcode($codeContents, 'QRCODE,Q', 120, 70, 25, 25, $style, 'N');
//	
...
if(!empty($InsCotizacionVehiculo->CveFoto)){
	...
	$pdf->Image('../../../subidos/vehiculo_version_fotos/'.utf8_decode($InsCotizacionVehiculo->CveFoto),135,85,NULL,25);
	
}else if(!empty($InsCotizacionVehiculo->VveFoto)){

	$pdf->Image('../../../subidos/vehiculo_version_fotos/'.utf8_decode($InsCotizacionVehiculo->VveFoto),135,85,NULL,25);
...
if(!empty($InsCotizacionVehiculo->CveFoto) or !empty($InsCotizacionVehiculo->VveFoto)){
	...
	$pdf->SetFont('times','',6);
	$pdf->Cell(110,3,'',$Borde,0,'C');
	$pdf->SetFont('times','',6);
	$pdf->Cell(80,3,'* Foto referencial',$Borde,1,'C');
...
if(!empty($InsCotizacionVehiculo->VveFotoCaracteristica)){
	...
	$pdf->Image('../../../subidos/vehiculo_version_fotos/'.utf8_decode($InsCotizacionVehiculo->VveFotoCaracteristica),'','',190,110,NULL,NULL,NULL,true,300,NULL,false,false,0,false,false,false,false);
...
if(!empty($InsCotizacionVehiculo->CveObservacion)){
	...
	if($TotalPalabras>$LimitePalabras){
		...
		$pdf->SetFont('times','',8);
		$pdf->Cell(100,5,html2text($InsCotizacionVehiculo->CveObservacion),$Borde,0,'L');
...
if(!empty($InsCotizacionVehiculo->CveFechaVigencia)){
	...
	$pdf->Cell(190,5,('- Cotización válida hasta el '.$InsCotizacionVehiculo->CveFechaVigencia),$Borde,1,'L');
...
//$pdf->Cell(38,3,'',$Borde,1,'L');
	  ...
	  if(file_exists("../cnf/Cuentas".$InsCotizacionVehiculo->SucSiglas."X.php")){
		  include("../cnf/Cuentas".$InsCotizacionVehiculo->SucSiglas."X.php");
	  }
