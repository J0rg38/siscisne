...
$().ready(function() {
	...
	$.ajax({
		type: 'GET',
		dataType: 'json',
		url: 'acc/AccGuiaRemisionGenerarBajaXMLv2.php',
		data: 'Id=<?php echo $InsGuiaRemision->GreId;?>&Ta=<?php echo $InsGuiaRemision->GrtId;?>',
			...
			switch(respuesta['CodigoRespuesta']){
				case "1":
					...
					?>	
						...
						window.setTimeout("FncGuiaRemisionProcesarBajaXML('"+respuesta['Id']+"');",2500);
					<?php	
					}else{
					?>
						$('#CapGuiaRemisionAccion').html('Archivo xml de baja generado correctamente...');
...
function FncGuiaRemisionProcesarBajaXML(oId){
	
	FncPopUp("FrmGuiaRemisionProcesarBajaXMLv2.php?Id="+oId+"&Procesar=<?php echo $GET_Procesar?>&EnviarSUNAT=<?php echo $GET_EnviarSUNAT;?>&P=1",0,0,1,0,0,1,0,350,150);
