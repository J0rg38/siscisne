...
function FncValidar(){
	...
	var NumeroRecibo = $("#CmpNumeroRecibo").val();
	...
	var TipoCambio = $("#CmpTipoCambio").val();	
		...
		if(AreaId == ""){		
			...
			return false;
				...
				dhtmlx.alert({
					title:"Aviso",
					type:"alert-error",
					text:"Debes ingresar un numero de recibo",
					callback: function(result){
						$("#CmpNumeroRecibo").focus();
					}
				});
		...
		}else if(FncValidarFecha(FechaTransaccion) == false && FechaTransaccion != ""){			
...
$().ready(function() {
	...
	$("input,select,textarea").keypress(function (event) {  
		 if (event.keyCode == '13' && this.type !== "hidden") {
			FncPagoVentaDirectaNavegar(this.id);
		 }
	}); 
...
*/
	...
	$("select#CmpMonedaId").change(function(){
		FncPagoVentaDirectaEstablecerMoneda();
		FncCuentaCargar();
		FncTarjetaCargar();
	});
	
	$("select#CmpCuenta").change(function(){
		FncPagoVentaDirectaEstablecerCuenta();
	});
...
function FncPagoVentaDirectaNavegar(oCampo){
...
function FncPagoVentaDirectaEstablecerMoneda(){
...
function FncPagoVentaDirectaEstablecerCuenta(){
