...
<tr>
  ...
  <td width="9%" align="left" valign="top"><?php
	if($POST_Borrar=="1"){
	?>
    <?php $_SESSION['SesEinArchivoDAM2'.$Identificador] = ""; ?>
    <?php	
	}
?>
    <form method="post" enctype="multipart/form-data">
...
if (!empty($_FILES)) {
	...
	if($extension=="jpg" || $extension == "png" || $extension == "gif" || $extension == "jpeg"){
		if (move_uploaded_file($tempFile,$targetFile)){
			...
			$_SESSION['SesEinArchivoDAM2'.$Identificador] = $file_name;
...
?>
  Solo se permiten ARCHIVOS ADJUNTOS de imagen con extension: jpg, jpeg, png y gif.
  <?php
	}
	
}elseif(!empty($_SESSION['SesEinArchivoDAM2'.$Identificador])){
	
	$extension = strtolower(pathinfo($_SESSION['SesEinArchivoDAM2'.$Identificador], PATHINFO_EXTENSION));
	$nombre_base = basename($_SESSION['SesEinArchivoDAM2'.$Identificador], '.'.$extension);  
?>
