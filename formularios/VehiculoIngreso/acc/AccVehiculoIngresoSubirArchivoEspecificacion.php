...
if (!empty($_FILES)) {
	...
	if($extension=="jpg" || $extension == "png" || $extension == "gif" || $extension == "jpeg" || $extension=="pdf" || $extension=="doc" || $extension=="docx" || $extension=="xls" || $extension=="xlsx"){
		...
		if (move_uploaded_file($tempFile,$targetFile)){
			...
			$_SESSION['SesEinEspecificacion'.$Identificador] = $file_name;
?>
Vista Previa:<br />
...
}elseif(!empty($_SESSION['SesEinEspecificacion'.$Identificador])){
	
	$extension = strtolower(pathinfo($_SESSION['SesEinEspecificacion'.$Identificador], PATHINFO_EXTENSION));
	$nombre_base = basename($_SESSION['SesEinEspecificacion'.$Identificador], '.'.$extension);  
?>
