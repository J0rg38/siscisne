...
<script type="text/javascript" src="<?php echo $InsProyecto->MtdFormulariosJs($GET_mod);?>JsCitaVerCalendarioFullFunciones.js"></script>

<style type="text/css">
@import url('<?php echo $InsProyecto->MtdFormulariosCss($GET_mod);?>CssCitaCalendario.css');
</style>
...
var FechaHoy = new Date(<?php echo date("Y")?>,<?php echo date("n")-1?>,<?php echo date("j")?>);
	...
	//function init() {
		...
		scheduler.init('scheduler_here',oDate,"week");
			...
			tb_show('Ver Cita','principal2.php?Mod=Cita&Form=Ver&Id='+event.subid+'&Dia=1&placeValuesBeforeTB_=savedValues&TB_iframe=true&height=500&width=890&modal=false',this.rel);		

			return true;
  		});
		
		
		//scheduler.load("formularios/Cita/data/events.xml");
		scheduler.load("formularios/Cita/data/citas.php?Personal="+oPersonal+"&PersonalMecanico="+oPersonalMecanico+"&Sucursal="+oSucursal);
		//scheduler.load("formularios/Cita/data/citas.php");
	}
...
<tr>
  <td width="98%" height="25">
    ...
    <span class="EstFormularioTitulo">VER CALENDARIO DE CITAS </span></td>
