...
if(isset($_POST['BtnEnviarCorreo_x']) or $_POST['Guardar']=="1"){	
	...
	$InsOrdenCompra->OcoDestinatarios = $_POST['CmpDestinatario'];
...
function FncCargarDatos(){
	...
	$InsOrdenCompra->OcoDestinatarios = "chevrolet.carperu@gm.com, pcondori@cyc.com.pe, jblanco@cyc.com.pe, aliendo@cyc.com.pe, iquezada@cyc.com.pe, scanepam@cyc.com.pe, bernardo.marcos@gm.com, richard.duenas@gm.com, dvercelone@cyc.com.pe, anace@cyc.com.pe, jean.anca@gm.com";
//
//	if($InsOrdenCompra->OcoTipo == "STX"){
//		$InsOrdenCompra->OcoDestinatarios .= ", mzamora@cyc.com.pe";
//	}
	...
	if(!empty($InsOrdenCompra->OrdenCompraPedido)){
		foreach($InsOrdenCompra->OrdenCompraPedido as $DatOrdenCompraPedido){
			...
			if(!empty($InsPedidoCompra->PerEmail)){
				...
				$ArrCorreos = explode(",",$InsOrdenCompra->OcoDestinatarios);
				
				if(!in_array($InsPedidoCompra->PerEmail,$ArrCorreos)){
					$InsOrdenCompra->OcoDestinatarios .= ",".$InsPedidoCompra->PerEmail;
				}
