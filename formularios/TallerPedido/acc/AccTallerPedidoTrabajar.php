...
$InsFichaIngreso->FinNota = addslashes($_POST['CmpNota']);	
	...
	//Parametro7 = FilTiempoCreacion
	//Parametro8 = FilTiempoModificacion
	//Parametro9 = PrvNombre
	//Parametro10 = PrvApellidoPaterno
	//Parametro11 = PrvApellidoMaterno
	//Parametro12 = MonNombre
	...
	if(!empty($InsFichaIngreso->FichaIngresoModalidad)){
		foreach($InsFichaIngreso->FichaIngresoModalidad as $DatFichaIngresoModalidad){
			...
			//deb($InsFichaAccion->FccManoObra);
		//	deb($TipoCambio);
			...
			$InsTallerPedido->TallerPedidoDetalle = array();
				...
				$ArrTallerPedidoDetalles = $RepSesionObjetos['Datos'];
					
				$item = 1;
				if(!empty($ArrTallerPedidoDetalles)){
					foreach($ArrTallerPedidoDetalles as $DatSesionObjeto){
...
//						deb($DatFichaIngresoModalidad->MinSigla." - ".$InsTallerPedidoDetalle1->ProId." - ".$InsTallerPedidoDetalle1->AmdEstado);
...
//						if($InsTallerPedidoDetalle1->AmdEliminado == 1 && $InsTallerPedidoDetalle1->AmdEstado == 3){
...
if($InsTallerPedidoDetalle1->AmdEliminado == 1){
				...
				$ArrTallerPedidoMantenimientos = $RepSesionObjetos['Datos'];
				
				$item = 1;
				if(!empty($ArrTallerPedidoMantenimientos)){
					foreach($ArrTallerPedidoMantenimientos as $DatSesionObjeto){
...
//							
						...
						//$InsTallerPedidoDetalle1->AmdCantidadReal = '';
							...
							//$InsTallerPedidoDetalle1->AmdReingreso = $_POST['Cmp'.$DatSesionObjeto->Parametro21.'TallerPedidoDetalleReingreso'];
							$InsTallerPedidoDetalle1->AmdReingreso = 2;
							//$InsTallerPedidoDetalle1->AmdCompraOrigen = $_POST['CmpTallerPedidoDetalleCompraOrigen_'.$Parametro21->Parametro21];
							$InsTallerPedidoDetalle1->AmdCompraOrigen = "X";
				...
				//	Parametro34 = AmoCierre
					...
					if(!empty($InsFichaAccion->FichaAccionProducto)){
						foreach($InsFichaAccion->FichaAccionProducto as $DatFichaAccionProducto){					
							...
							if(!empty($DatFichaAccionProducto->ProId) and ($DatFichaAccionProducto->FapAccion=="C" or $DatFichaAccionProducto->FapAccion=="U" or  $DatFichaAccionProducto->FapAccion=="") ){
...
//MtdObtenerProductoTipoUnidadMedidas($oCampo=NULL,$oCondicion=NULL,$oFiltro=NULL,$oOrden = 'PtuId',$oSentido = 'Desc',$oPaginacion = '0,10',$oTipo=NULL,$oProductoTipo=NULL,$oUso=NULL)
								...
								or $DatFichaIngresoModalidad->MinSigla == "PP" 
									...
									//deb($InsTallerPedidoDetalle1->AmdPrecioVenta);
...
//							
//							$InsFichaAccion1 = new ClsFichaAccion();
//							$InsFichaAccion1->FccId = $InsFichaAccion->FccId;
//							$InsFichaAccion1->FimId = $InsFichaAccion->FimId;
//							
//							//deb($DatFichaIngresoModalidad->FichaIngresoMantenimiento);
//							
//							
//							if(!empty($DatFichaIngresoModalidad->FichaIngresoMantenimiento)){
//								foreach($DatFichaIngresoModalidad->FichaIngresoMantenimiento as $DatFichaIngresoMantenimiento){
...
//										$InsFichaAccionMantenimiento1->InsMysql = NULL;
//			
//										$InsFichaAccion1->FichaAccionMantenimiento[] = $InsFichaAccionMantenimiento1;
//										
//									}
//				
//								}
//								
//								if(!empty($InsFichaAccion1->FichaAccionMantenimiento)){
//									$InsFichaAccion1->MtdCorregirFichaAccionMantenimiento();
//								}
//								
//								
//							}
...
//	if($InsFichaIngreso->FinEstado == 71 or $InsFichaIngreso->FinEstado == 7){
//		//ACTUALIZANDO A ALMACEN [Pedido Extornado]
//		$InsFichaIngreso->MtdActualizarEstadoFichaIngreso($InsFichaIngreso->FinId,72,false); //OJOO
//	}
//	
}
...
//					deb($InsTallerPedido->AmoFecha."");
		...
		//////deb($DatTallerPedidoDetalle->AmdId);
	...
	//										deb($InsTallerPedido->MonId." - ".$EmpresaMonedaId);
											if($InsTallerPedido->MonId<>$EmpresaMonedaId ){
...
function FncCargarTallerPedidoHerramientaDatos(){
