...
$().ready(function() {
	...
	$('#BtnVer').on('click', function() {
		if(FncValidar()){
			FncReporteComprobanteCobrarVer('');
		}
	});
	...
	$('#BtnImprimir').on('click', function() {
		if(FncValidar()){
			FncReporteComprobanteCobrarImprimir('');
		}
	});
	...
	$('#BtnExcel').on('click', function() {
		if(FncValidar()){
			FncReporteComprobanteCobrarGenerarExcel('');
		}
	});
...
function FncReporteComprobanteCobrarImprimir(oIndice){
	
	
	FncObtenerVariables();
	
	var Accion = document.getElementById('FrmReporteComprobanteCobrar'+oIndice).action;
	
	FncPopUp(Accion+"?"+Variables+"&P=1",0,0,1,0,0,1,0,screen.height,screen.width);
	
}

function FncReporteComprobanteCobrarGenerarExcel(oIndice){
...
function FncReporteComprobanteCobrarVer(oIndice){
	
	document.getElementById('FrmReporteComprobanteCobrar'+oIndice).submit();
