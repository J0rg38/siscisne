...
$().ready(function() {
	...
	$('#BtnVer').on('click', function() {
		if(FncValidar()){
			FncReporteFichaIngresoSinFacturarVer('');
		}
	});
	...
	$('#BtnImprimir').on('click', function() {
		if(FncValidar()){
			FncReporteFichaIngresoSinFacturarImprimir('');
		}
	});
	...
	$('#BtnExcel').on('click', function() {
		if(FncValidar()){
			FncReporteFichaIngresoSinFacturarGenerarExcel('');
		}
	});
...
function FncReporteFichaIngresoSinFacturarImprimir(oIndice){
	
	
	FncObtenerVariables();
	
	var Accion = document.getElementById('FrmReporteFichaIngresoSinFacturar'+oIndice).action;
	
	FncPopUp(Accion+"?"+Variables+"&P=1",0,0,1,0,0,1,0,screen.height,screen.width);
	
}

function FncReporteFichaIngresoSinFacturarGenerarExcel(oIndice){
...
function FncReporteFichaIngresoSinFacturarVer(oIndice){
	
	document.getElementById('FrmReporteFichaIngresoSinFacturar'+oIndice).submit();
