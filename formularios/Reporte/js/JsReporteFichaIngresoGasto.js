...
$().ready(function() {
	...
	$('#BtnVer').on('click', function() {
		if(FncValidar()){
			FncReporteFichaIngresoGastoVer('');
		}
	});
	...
	$('#BtnImprimir').on('click', function() {
		if(FncValidar()){
			FncReporteFichaIngresoGastoImprimir('');
		}
	});
	...
	$('#BtnExcel').on('click', function() {
		if(FncValidar()){
			FncReporteFichaIngresoGastoGenerarExcel('');
		}
	});
...
function FncReporteFichaIngresoGastoImprimir(oIndice){
	
	
	FncObtenerVariables();
	
	var Accion = document.getElementById('FrmReporteFichaIngresoGasto'+oIndice).action;
	
	FncPopUp(Accion+"?"+Variables+"&P=1",0,0,1,0,0,1,0,screen.height,screen.width);
	
}

function FncReporteFichaIngresoGastoGenerarExcel(oIndice){
...
function FncReporteFichaIngresoGastoVer(oIndice){
	
	document.getElementById('FrmReporteFichaIngresoGasto'+oIndice).submit();
