...
include("IfrReporteEncuestaPostVenta.php");
exit();
...
$ArrSucursales = $RepSucursal['Datos'];
 ...
 // Create new PHPExcel object
  ...
  $objReader = PHPExcel_IOFactory::createReader('Excel5');
			...
			$Titulo = "REPORTE DE ENCUESTAS DE POST VENTA  DEL ";
...
if(!empty($ArrEncuestaPreguntaSecciones)){
	...
	$ArrEncuestaPreguntas = $ResEncuestaPregunta['Datos'];
		...
		$objPHPExcel->setActiveSheetIndex(1)
			->setCellValue("B".$fila, $DatEncuestaPreguntaSeccion->EpsNombre);//H1
		...
		$fila = $fila + 2;
			...
			if(!empty($ArrEncuestaPreguntas )){
				...
				if($DatEncuestaPregunta->EprTipo=="1" || $DatEncuestaPregunta->EprTipo=="3"){
					...
					$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue("B".$fila, $DatEncuestaPregunta->EprNombre);//H2
			...
			$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue("B".$fila, "Pregunta");//H2
			...
			if(!empty($ArrEncuestaPreguntaRespuestas)){
				foreach($ArrEncuestaPreguntaRespuestas as $DatEncuestaPreguntaRespuesta){
					...
					if($DatEncuestaPregunta->EprTipo=="1" || $DatEncuestaPregunta->EprTipo=="3"){
                        ...
                        $objPHPExcel->setActiveSheetIndex(1)
                            ->setCellValue(FncConvertirNumeroALetraExcel($Columna).$fila, $DatEncuestaPreguntaRespuesta->EpeNombre);
...
?> 
     ...
     <img src="<?php  echo (($_GET['P']==1)?$InsProyecto->MtdRutGenerados():'');?>reportes/<?php echo $filename;?>">   
...
$InsEncuesta = new ClsEncuesta();
$ResEncuesta = $InsEncuesta->MtdObtenerEncuestas(NULL,NULL,NULL,"EncId","DESC",$POST_pag,NULL,FncCambiaFechaAMysql($POST_FechaInicio),FncCambiaFechaAMysql($POST_FechaFin),"POSTVENTA",$DatSucursal->SucId);
...
$InsEncuesta = new ClsEncuesta();
$ResEncuesta = $InsEncuesta->MtdObtenerEncuestas(NULL,NULL,NULL,"EncId","DESC",$POST_pag,NULL,FncCambiaFechaAMysql($POST_FechaInicio),FncCambiaFechaAMysql($POST_FechaFin),"POSTVENTA",$POST_Sucursal);
$ArrEncuestas = $ResEncuesta['Datos'];
...
?>
        ...
        $objWriter->save("../../generados/reportes/REPORTE_ENCUESTA_POSTVENTA__".date("Y")."_".date("m")."_".date("d").".xls");
        
        //$objWriter->save(str_replace('.php', '.xls', __FILE__));
        /*
        <a href="<?php echo $InsOrdenCompra->OcoId;?>.xls">DESCARGAR: <?php echo $InsOrdenCompra->OcoId;?>.xls</a>
        */
        header("Location: ../../generados/reportes/REPORTE_ENCUESTA_POSTVENTA_".date("Y")."_".date("m")."_".date("d").".xls");
