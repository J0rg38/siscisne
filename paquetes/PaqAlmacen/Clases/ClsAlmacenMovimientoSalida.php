...
//	public $CliApellidoMaterno;
    ...
    public function MtdObtenerAlmacenMovimientoSalida(){
        ...
        while ($fila = $this->InsMysql->MtdObtenerDatos($resultado))
        {
			...
			$this->AmoTipo = $fila['AmoTipo'];
			$this->AmoSubTipo = $fila['AmoSubTipo'];
			
			$this->AmoTipoMovimiento = $fila['AmoTipoMovimiento'];
...
//		}
		...
		if(!empty($oSubTipo)){
			...
			$sdestino = ' AND amo.SucIdDestino = "'.$oSucursalDestino.'"';
		}
									...
									ON fim.MinId = min.MinId
										...
										ON amo.MonId = mon.MonId
											...
											ON amo.SucId = suc.SucId
												...
												ON amo.SucIdDestino = suc2.SucId
				WHERE amo.AmoTipo = 2 
            ...
            $InsAlmacenMovimientoSalida = get_class($this);
				...
				while( $fila = $this->InsMysql->MtdObtenerDatos($resultado)){
					...
					$AlmacenMovimientoSalida->SucId = $fila['SucId'];
					$AlmacenMovimientoSalida->SucIdDestino = $fila['SucIdDestino'];
					...
					$AlmacenMovimientoSalida->AmoTipoMovimiento = $fila['AmoTipoMovimiento'];
				...
				$AlmacenMovimientoSalida->SucNombre = $fila['SucNombre'];
				$AlmacenMovimientoSalida->SucNombreDestino = $fila['SucNombreDestino'];
			...
			if(!$error){			
				...
				if (!empty($this->AlmacenMovimientoSalidaDetalle)){		
					...
					$InsProducto = new ClsProducto();
						...
						//$StockReal = $InsAlmacenProducto->MtdObtenerAlmacenProductoStockActual($InsTallerPedidoDetalle->ProId,$this->AlmId,$Ano,$this->SucId);
						$StockReal = $InsAlmacenStock->MtdObtenerAlmacenStockProductoStockReal($this->SucId,$this->AlmId,$Ano,$InsAlmacenMovimientoSalidaDetalle->ProId);
							$StockReal = 10000;
						//deb($StockReal);
						...
						if( $StockReal < $InsAlmacenMovimientoSalidaDetalle->AmdCantidadReal and $InsAlmacenMovimientoSalidaDetalle->AmdEliminado == 1 and $InsAlmacenMovimientoSalidaDetalle->AmdEstado == 3){
...
//			}
	...
	public function MtdEditarAlmacenMovimientoSalida() {
		...
		AmoTipoMovimiento = "'.($this->AmoTipoMovimiento).'",
