...
class ClsVehiculoMovimientoSalida {
		...
		if(!empty($oSubTipo)){
			$stipo = ' AND vmv.VmvSubTipo = "'.$oSubTipo.'"';
			 ...
			 $sql = 'SELECT
				...
				WHERE 1 = 1 AND vmv.VmvTipo = 2 '.$filtrar.$fecha.$stipo.$estado.$origen.$moneda.$pcompra.$ocompra.$pcompradetalle.$cliente.$cocompra.$cancelado.$einveedor.$vdirecta.$cpago.$almacen.$orden.$paginacion;
            ...
            $InsVehiculoMovimientoSalida = get_class($this);
				...
				while( $fila = $this->InsMysql->MtdObtenerDatos($resultado)){
					...
					$VehiculoMovimientoSalida->PrvId = $fila['PrvId'];		
					...
					$VehiculoMovimientoSalida->PrvNombreCompleto = $fila['PrvNombreCompleto']; 
					$VehiculoMovimientoSalida->PrvNombre = $fila['PrvNombre']; 
					$VehiculoMovimientoSalida->PrvApellidoPaterno = $fila['PrvApellidoPaterno']; 
					$VehiculoMovimientoSalida->PrvApellidoMaterno = $fila['PrvApellidoMaterno']; 
					
					$VehiculoMovimientoSalida->PrvNumeroDocumento = $fila['PrvNumeroDocumento']; 
	...
	public function MtdRegistrarVehiculoMovimientoSalida() {
		...
		$error = false;
			...
			$VehiculoMovimientoSalidaId = $this->MtdVerificarExisteVehiculoMovimientoSalida("VmvComprobanteNumero",$this->VmvComprobanteNumero,$this->PrvId);
...
//			}
	...
	public function MtdEditarVehiculoMovimientoSalida() {
		...
		$error = false;
			...
			VmvComprobanteNumero = "'.($this->VmvComprobanteNumero).'",
...
//			}
		...
		private function MtdAuditarVehiculoMovimientoSalida($oAccion,$oDescripcion,$oDatos){
		...
		global $SistemaNombreAbreviado;
			...
			foreach($ArrMonedas as $DatMoneda){
				...
				if($Mostrar){
					...
					$mensaje .= "<td>";
					$mensaje .= $DatVehiculoMovimientoSalida->PrvNumeroDocumento;
					$mensaje .= "</td>";

					
					$mensaje .= "<td>";
					$mensaje .= $DatVehiculoMovimientoSalida->PrvNombreCompleto;
					$mensaje .= "</td>";
