...
*/
		...
		if (!$error) {
			...
			if (!empty($this->CprId)) {
				...
				if (!empty($this->FinId)) {
					...
					$InsCotizacionProducto = new ClsCotizacionProducto($this->InsMysql);
		...
		if (!$error) {
			...
			if (!empty($this->VentaDirectaDetalle)) {
				...
				$validar = 0;
				$InsVentaDirectaDetalle = new ClsVentaDirectaDetalle($this->InsMysql);
		...
		if (!$error) {
			...
			if (!empty($this->VentaDirectaPlanchado)) {
				...
				$InsVentaDirectaPlanchado = new ClsVentaDirectaTarea($this->InsMysql);
		...
		if (!$error) {
			...
			if (!empty($this->VentaDirectaPintado)) {
				...
				$InsVentaDirectaPintado = new ClsVentaDirectaTarea($this->InsMysql);
		...
		if (!$error) {
			...
			if (!empty($this->VentaDirectaCentrado)) {
				...
				$InsVentaDirectaCentrado = new ClsVentaDirectaTarea($this->InsMysql);
		...
		if (!$error) {
			...
			if (!empty($this->VentaDirectaTarea)) {
				...
				$InsVentaDirectaTarea = new ClsVentaDirectaTarea($this->InsMysql);
		...
		if (!$error) {
			...
			if (!empty($this->VentaDirectaFoto)) {
				...
				foreach ($this->VentaDirectaFoto as $DatVentaDirectaFoto) {
					...
					$InsVentaDirectaFoto = new ClsVentaDirectaFoto($this->InsMysql);
	...
	public function MtdGenerarVentaConcretadas($oElementos, $oEstricto = false)
	{
		...
		foreach ($elementos as $elemento) {
			...
			if (!empty($elemento)) {
				...
				$InsVentaConcretada = new ClsVentaConcretada($this->InsMysql);
	...
	public function MtdGenerarVentaConcretada($oVentaDirectaId)
	{
		...
		if (!empty($oVentaDirectaId)) {
			...
			$InsUnidadMedida = new ClsUnidadMedida($this->InsMysql);
			...
			$InsUnidadMedidaConversion = new ClsUnidadMedidaConversion($this->InsMysql);
			$InsUnidadMedida = new ClsUnidadMedida($this->InsMysql);
					...
					if ($DatVentaDirectaDetalle->VddCantidadPendiente2 > 0) {
						...
						if ($DatVentaDirectaDetalle->VddEstado == 1) {
							...
							if ($GuardarDetalle) {
								...
								$InsVentaConcretadaDetalle1 = new ClsVentaConcretadaDetalle($this->InsMysql);
	...
	public function MtdVentaDirectaGenerarPedidoCompras($oElementos, $oGenerarOrdenCompra = false, $oUsuario = NULL, $oEnviarNotificacion = false, $oDestinatariosNotificar = NULL, $oEnviarAdjunto = false, $oRuta = "../../", $oDestinatariosEnvio = NULL, $IgnorarStock = true)
	{
		...
		foreach ($elementos as $elemento) {
			...
			if (!empty($elemento)) {
				...
				$InsPedidoCompra = new ClsPedidoCompra($this->InsMysql);
				...
				if (!empty($this->VentaDirectaDetalle)) {
					foreach ($this->VentaDirectaDetalle as $DatVentaDirectaDetalle) {
						...
						if (!empty($DatVentaDirectaDetalle->ProCodigoOriginal)) {
							...
							$InsProductoListaPrecio = new ClsProductoListaPrecio($this->InsMysql);
						...
						$InsProductoDisponibilidad = new ClsProductoDisponibilidad($this->InsMysql);
						...
						$Reemplazo = "NO";
						$InsProductoReemplazo = new ClsProductoReemplazo($this->InsMysql);
						...
						if ($GuardarDetalle) {
							...
							$InsPedidoCompraDetalle1 = new ClsPedidoCompraDetalle($this->InsMysql);
					...
					if ($InsPedidoCompra->MtdRegistrarPedidoCompra(false)) {
						...
						if ($oGenerarOrdenCompra) {
							...
							if (!empty($this->VdiTipoPedido)) {
								...
								$InsOrdenCompra = new ClsOrdenCompra($this->InsMysql);
								...
								$InsProveedor = new ClsProveedor($this->InsMysql);
								...
								$InsTipoCambio = new ClsTipoCambio($this->InsMysql);
