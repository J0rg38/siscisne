...
class ClsTrasladoVehiculo {
    ...
    public function MtdObtenerTrasladoVehiculo(){
		...
		tve.MonId,
		tve.TveTipoCambio,
		tve.TveIncluyeImpuesto,
		tve.TvePorcentajeImpuestoVenta,
		
		DATE_FORMAT(tve.TveFecha, "%d/%m/%Y") AS "NTveFecha",
		DATE_FORMAT(tve.TveFechaLlegada, "%d/%m/%Y") AS "NTveFechaLlegada",
		...
		//	deb("");
			//deb("");
//			deb($fila['SucId']);
//			deb($fila['SucIdDestino']);
//			
			$this->PerId = $fila['PerId'];	
			
			//	deb($this->SucId);
			//deb($this->SucIdDestino);
			...
			$this->TveTipoCambio = $fila['TveTipoCambio'];	
			$this->TveIncluyeImpuesto = $fila['TveIncluyeImpuesto'];				
			$this->TvePorcentajeImpuestoVenta = $fila['TvePorcentajeImpuestoVenta'];	
			
			$this->TveFecha = $fila['NTveFecha'];
			$this->TveFechaLlegada = $fila['NTveFechaLlegada'];
...
$this->PerNombre = $fila['PerNombre'];
		...
		tve.TvePorcentajeImpuestoVenta,
            ...
            $InsTrasladoVehiculo = get_class($this);
				...
				while( $fila = $this->InsMysql->MtdObtenerDatos($resultado)){
					...
					$TrasladoVehiculo->CliId = $fila['CliId'];	
					$TrasladoVehiculo->PrvId = $fila['PrvId'];	
					$TrasladoVehiculo->CtiId = $fila['CtiId'];	
					$TrasladoVehiculo->TopId = $fila['TopId'];	
					$TrasladoVehiculo->MonId = $fila['MonId'];	
					$TrasladoVehiculo->TveTipoCambio = $fila['TveTipoCambio'];	
					$TrasladoVehiculo->TveIncluyeImpuesto = $fila['TveIncluyeImpuesto'];	
					$TrasladoVehiculo->TvePorcentajeImpuestoVenta = $fila['TvePorcentajeImpuestoVenta'];	
					
					$TrasladoVehiculo->TveFecha = $fila['NTveFecha'];
					$TrasladoVehiculo->TveFechaLlegada = $fila['NTveFechaLlegada'];
				...
				if(!empty($elemento)){
					...
					if(!empty($ArrTrasladoVehiculoDetalles)){
						...
						foreach($ArrTrasladoVehiculoDetalles as $DatTrasladoVehiculoDetalle){
							$tvdetalle .= '#'.$DatTrasladoVehiculoDetalle->TvdId;
						}

						if(!$InsTrasladoVehiculoDetalle->MtdEliminarTrasladoVehiculoDetalle($tvdetalle)){								
							$error = true;
						}
	...
	public function MtdRegistrarTrasladoVehiculo() {
		...
		$error = false;
			...
			"'.($this->MonId).'", 
			'.(empty($this->TveTipoCambio)?'NULL, ':'"'.$this->TveTipoCambio.'",').'
			'.($this->TveIncluyeImpuesto).', 
			'.($this->TvePorcentajeImpuestoVenta).', 
			
			"'.($this->TveFecha).'", 
			"'.($this->TveFechaLlegada).'", 
			...
			if(!$error){			
				...
				if (!empty($this->TrasladoVehiculoDetalle)){		
					...
					foreach ($this->TrasladoVehiculoDetalle as $DatTrasladoVehiculoDetalle){
						...
						$InsTrasladoVehiculoDetalle->VehId = $DatTrasladoVehiculoDetalle->VehId;
						...
						$InsTrasladoVehiculoDetalle->TvdCosto = $DatTrasladoVehiculoDetalle->TvdCosto;
						$InsTrasladoVehiculoDetalle->TvdImporte = $DatTrasladoVehiculoDetalle->TvdImporte;
						
						$InsTrasladoVehiculoDetalle->TvdDescripcion = $DatTrasladoVehiculoDetalle->TvdDescripcion;
	...
	public function MtdEditarTrasladoVehiculo() {
		...
		$error = false;
			...
			MonId = "'.($this->MonId).'",
			'.(empty($this->TveTipoCambio)?'TveTipoCambio = NULL, ':'TveTipoCambio = "'.$this->TveTipoCambio.'",').'	
			
			TveIncluyeImpuesto = '.($this->TveIncluyeImpuesto).',
			TvePorcentajeImpuestoVenta = '.($this->TvePorcentajeImpuestoVenta).',
	
			TveFecha = "'.($this->TveFecha).'",
			TveFechaLlegada = "'.($this->TveFechaLlegada).'",
