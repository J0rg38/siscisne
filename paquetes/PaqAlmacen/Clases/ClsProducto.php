...
class ClsProducto
{
	...
	public $ProUbicacion;
	public $ProMarcaReferencia;
	public $ProNota;
	public $ProDisponibilidadCantidadGM;
	public $ProRevisadoFecha;
	public $ProListaPrecioPrecio;
	public $ProTienePromocion;
	public $UmeIdSalida;
	public $ProPorcentajeAdicional;
	public $ProReemplazo;

	// Propiedades adicionales para evitar warnings
	public $ProCritico;
	public $ProDescontinuado;
	public $ProPromedioDiario;
	public $ProPromedioMensual;
	public $ProPromedioTrimestral;
	public $ProPromedioSemestral;
	public $ProPromedioAnual;
	public $ProSalidaTotalAnual;
	public $ProSalidaTotalTrimestral;
	public $ProSalidaTotalSemestral;

	public $ProFechaUltimaSalida;
	public $ProDiasInmovilizado;

	public $ProFechaUltimaActividad;
	public $ProFechaUltimaEntrada;
	public $ProEspecificacion;
...
pro.ProEstado,
		...
		if ($this->InsMysql->MtdObtenerDatosTotal($resultado) > 0) {
			...
			while ($fila = $this->InsMysql->MtdObtenerDatos($resultado)) {
				...
				$this->ProMarcaReferencia = htmlentities(utf8_decode($fila['ProMarcaReferencia']));
				...
				$this->ProReemplazo = $fila['ProReemplazo'];
	...
	public function MtdObtenerProductos($oCampo = NULL, $oCondicion = NULL, $oFiltro = NULL, $oOrden = 'ProId', $oSentido = 'Desc', $oPaginacion = '0,10', $oEstado = NULL, $oTipo = NULL, $oValidarStock = 1, $oVehiculoMarca = NULL, $oVehiculoModelo = NULL, $oVehiculoVersion = NULL, $oVehiculoAno = NULL, $oTieneIngreso = false, $oReferencia = NULL, $oFecha = NULL, $oTieneSock = 0, $oProductoCategoria = NULL, $oUsoEstricto = false, $oCalcularPrecio = NULL, $oTieneCodigoOriginal = false, $oReemplazo = NULL)
	{
		...
		if (!empty($oReemplazo)) {
			$reemplazo = ' AND pro.ProReemplazo = "' . $oReemplazo . '"';
		}
...
ON amd.AmoId = amo.AmoId
WHERE amd.ProId = pro.ProId
AND amo.AmoTipo = 2 AND amo .AmoSubTipo !=6 AND amo.TopId <> "TOP-10015"
AND amo.AmoEstado = 3 AND amd.AmdEstado = 3
ORDER BY amd.AmdFecha DESC
LIMIT 1
),pro.ProFechaUltimaSalida 
)
AS "ProFechaUltimaSalida2",
...
DATE_FORMAT(pro.ProFechaUltimaEntrada, "%d/%m/%Y") AS "NProFechaUltimaEntrada",
DATE_FORMAT(pro.ProFechaUltimaSalida, "%d/%m/%Y") AS "NProFechaUltimaSalida",
				...
				WHERE  1 = 1  ' . $filtrar .   $reemplazo . $vmarcasolo . $tcoriginal . $cprecio . $referencia . $estado . $tipo . $vstock . $vmarca . $vmodelo . $vversion . $vano . $tingreso . $tstock . $pcategoria . $orden . $paginacion;
		...
		while ($fila = $this->InsMysql->MtdObtenerDatos($resultado)) {
			...
			$Producto->ProMarcaReferencia = ($fila['ProMarcaReferencia']);
			...
			$Producto->ProFechaUltimaSalida = $fila['ProFechaUltimaSalida2'];

			$Producto->ProReemplazo = $fila['ProReemplazo'];
			...
			$Producto->ProFechaUltimaEntrada = $fila['NProFechaUltimaEntrada'];
			$Producto->ProFechaUltimaSalida = $fila['NProFechaUltimaSalida'];
			...
			$Producto->VmaNombre = $fila['VmaNombre'];
	...
	public function MtdEditarProducto()
	{
		...
		ProMarcaReferencia = "' . ($this->ProMarcaReferencia) . '",
		...
		ProValidarStock = ' . ($this->ProValidarStock) . ',
		...
		if (!$error) {
			if (!empty($this->ListaPrecio)) {
				...
				foreach ($this->ListaPrecio as $DatListaPrecio) {
					...
					$InsListaPrecio->SucId = $this->SucId;
	...
	public function MtdEditarProductoStockMinimo()
	{
