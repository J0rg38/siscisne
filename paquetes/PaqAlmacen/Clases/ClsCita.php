...
class ClsCita {
	...
	public function MtdGenerarCitaId() {
			...
			if(empty($fila['MAXIMO'])){			
				$this->CitId = "CIT-10000-".(empty($fila['SIGLA'])?$_SESSION['SesionSucursalSiglas']:$fila['SIGLA']);

			}else{
				$fila['MAXIMO']++;
				$this->CitId = "CIT-".$fila['MAXIMO']."-".(empty($fila['SIGLA'])?$_SESSION['SesionSucursalSiglas']:$fila['SIGLA']);				
			}	
...
//		}	
		...
		if(!empty($oSucursal)){
			$sucursal = ' AND cit.EinId = "'.$oSucursal.'"';
		}
