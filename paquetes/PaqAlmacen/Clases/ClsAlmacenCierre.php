...
public function MtdVerificarAlmacenCierre($oFecha){
	
	$Cerrado = false;
	
	$ResAlmacenCierre = $this->MtdObtenerAlmacenCierres(NULL,NULL,NULL,'AciId','Desc','1',3,$oFecha);
	$ArrAlmacenCierres = $ResAlmacenCierre['Datos'];
	
	
	
	if(count($ArrAlmacenCierres)>0){
		$Cerrado = true;	
	}
    ...
    public function MtdObtenerAlmacenCierres($oCampo=NULL,$oCondicion=NULL,$oFiltro=NULL,$oOrden = 'AciId',$oSentido = 'Desc',$oPaginacion = '0,10',$oEstado=NULL,$oFechaComparar=NULL) {
		...
		if(!empty($oFechaComparar)){
			
			$fcomprar = ' AND ( aci.AciFechaInicio >= "'.$oFechaComparar.'" AND aci.AciFechaFin <= "'.$oFechaComparar.'" ) ';
		}
		
			$sql = 'SELECT
				SQL_CALC_FOUND_ROWS 
				aci.AciId,
				DATE_FORMAT(aci.AciFechaInicio, "%d/%m/%Y") AS "NAciFechaInicio",
				DATE_FORMAT(aci.AciFechaFin, "%d/%m/%Y") AS "NAciFechaFin",
				...
				WHERE 1 = 1 '.$filtrar.$moneda.$tipo.$estado.$fcomprar.$banco.$tarjeta.$orden.$paginacion;
