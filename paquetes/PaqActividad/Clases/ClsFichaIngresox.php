...
//cli.LtiId = "'.($elemento).'"
		...
		if(!empty($oAno)){
			$ano = ' AND YEAR(fin.FinFecha) ="'.($oAno).'"';
				...
				WHERE  FinMigrado IS NULL '.$ano.$mes.$filtrar.$sincosto .$sucursal.$fecha.$estado.$excluir.$prioridad.$mingreso.$vin.$cliente.$personal.$tconcluido.$campana.$cltipo.$tipo.$sexterna.$ccampana.$eingreso.$vmarca.$vmodelo.$mmkilometraje.$pasesor.$treparacion.$orden.$paginacion;
