        ...
        while ($fila = $this->InsMysql->MtdObtenerDatos($resultado))
        {
			...
			$Estado = "";
				...
				switch($this->FinEstado){
						...
						case 10:
							$Estado = "<img src='".$this->Ruta."imagenes/iconos/no_facturable.png' alt='NO FACTURABLE' border='0' width='20' height='20' title='NO FACTURABLE' > [No Facturable]";
						break;	
			...
			$InsFichaIngresoHerramienta = new ClsFichaIngresoHerramienta();
			...
			$InsFichaIngresoGasto = new ClsFichaIngresoGasto();
...
//			
			...
			$InsPreEntregaDetalle = new ClsPreEntregaDetalle();
...
CASE
		...
		onc.OncNombre
				...
				WHERE 1 = 1 '.$filtrar.$fecha.$estado.$coriginal.$prioridad.$vmarca.$pasesor.$mingreso.$vin.$cliente.$personal.$tconcluido.$campana.$treparacion.$sucursal.$cltipo.$tipo.$sexterna.$ccampana.$eingreso.$orden.$paginacion;
...
//cli.LtiId = "'.($elemento).'"
		...
		if(!empty($oAno)){
			$ano = ' AND YEAR(fin.FinFecha) ="'.($oAno).'"';
				...
				WHERE 1 = 1 '.$ano.$mes.$filtrar.$sincosto .$fecha.$estado.$excluir.$prioridad.$mingreso.$vin.$cliente.$personal.$tconcluido.$campana.$cltipo.$tipo.$sexterna.$ccampana.$eingreso.$vmarca.$vmodelo.$mmkilometraje.$pasesor.$treparacion.$orden.$paginacion;
...
//		}		
		...
		if(!empty($oAno)){
			...
			//echo "<br>";echo "<br>";
				...
				WHERE 1 = 1 '.$ano.$mes.$filtrar.$fecha.$vtipo.$estado.$prioridad.$sincosto.$mingreso.$vin.$cliente.$personal.$tconcluido.$campana.$cltipo.$vmarca.$vmodelo.$tipo.$orden.$paginacion;
...
//							}
	...
	//Accion eliminar	 
	public function MtdActualizarEstadoFichaIngreso($oElementos,$oEstado,$oTransaccion=true) {
		...
		//$InsFichaIngresoTareas = new ClsFichaIngresoTarea();
...
//			'.(empty($this->FinTipoCambio)?'FinTipoCambio = NULL, ':'FinTipoCambio = "'.$this->FinTipoCambio.'",').'
			...
			FinSalidaHora = "'.($this->FinSalidaHora).'",
...
if(!$error){
	...
	private function MtdAuditarFichaIngreso($oAccion,$oDescripcion,$oDatos){
		
		$InsAuditoria = new ClsAuditoria();
		$InsAuditoria->AudCodigo = $this->FinId;
