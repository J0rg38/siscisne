...
//	public $OrdenVentaVehiculoObsequio;
    ...
    public function MtdObtenerOrdenVentaVehiculo($oCompleto=true){
		...
		ein.EinCaracteristica20,
				...
				tdo4.TdoNombre AS TdoNombrePersonal,
							...
							ON vmo.VtiId = vti.VtiId	
									...
									ON vmo.VmaId = vma.VmaId
										...
										ON ovv.PerId = per.PerId
											...
											LEFT JOIN tbltdotipodocumento tdo4
											ON per.TdoId = tdo4.TdoId
        ...
        while ($fila = $this->InsMysql->MtdObtenerDatos($resultado))
        {
			...
			$this->OvvTarjeta = $fila['OvvTarjeta'];
			
			$this->OvvAprobacion1 = $fila['OvvAprobacion1'];
			$this->OvvAprobacion2 = $fila['OvvAprobacion2'];
			$this->OvvAprobacion3 = $fila['OvvAprobacion3'];
			
			$this->OvvReferencia = $fila['OvvReferencia'];
			$this->OvvTipoPlaca = $fila['OvvTipoPlaca'];
			$this->OvvGLPModeloTanque = $fila['OvvGLPModeloTanque'];
			
			$this->OvvInmediata = $fila['OvvInmediata'];
			$this->OvvEstado = $fila['OvvEstado'];
			
			$this->OvvTiempoSolicitudEnvio = $fila['NOvvTiempoSolicitudEnvio']; 
			$this->OvvTiempoAprobacion1Envio = $fila['NOvvTiempoAprobacion1Envio'];
			$this->OvvTiempoAprobacion2Envio = $fila['NOvvTiempoAprobacion2Envio'];
			$this->OvvTiempoEmitido = $fila['NOvvTiempoEmitido'];
			$this->OvvTiempoAnulado = $fila['NOvvTiempoAnulado'];
			$this->OvvTiempoPorFacturar = $fila['NOvvTiempoPorFacturar'];
			$this->OvvTiempoFacturado = $fila['NOvvTiempoFacturado'];
			...
			switch($this->OvvComprobanteVenta){
				
				case "F":
					$this->OvvComprobanteVentaDescripcion = "Factura";
				break;
				
				case "B":
					$this->OvvComprobanteVentaDescripcion = "Boleta";
				break;
				
				default:
					$this->OvvComprobanteVentaDescripcion = "-";
				break;
    ...
    public function MtdObtenerOrdenVentaVehiculos($oCampo=NULL,$oCondicion="contiene",$oFiltro=NULL,$oOrden = 'OvvId',$oSentido = 'Desc',$oPaginacion = '0,10',$oFechaInicio=NULL,$oFechaFin=NULL,$oEstado=NULL,$oMoneda=NULL,$oPersonal=NULL,$oCliente=NULL,$oConCotizacion=0,$oFacturable=NULL,$oCotizacionVehiculo=NULL,$oVehiculoIngreso=NULL,$oSucursal=NULL,$oAprobacion1=NULL,$oAprobacion2=NULL,$oAprobacion3=NULL,$oTieneActaFechaEntrega=0,$oTieneComprobante=false,$oFechaTipo="OvvFecha") {
		...
		if(!empty($oFechaInicio)){
			...
			if(!empty($oFechaFin)){
				$fecha = ' AND DATE(ovv.'.$oFechaTipo.')>="'.$oFechaInicio.'" AND DATE(ovv.'.$oFechaTipo.')<="'.$oFechaFin.'"';
			}else{
				$fecha = ' AND DATE(ovv.'.$oFechaTipo.')>="'.$oFechaInicio.'"';
			}
			
		}else{
			if(!empty($oFechaFin)){
				$fecha = ' AND DATE(ovv.'.$oFechaTipo.')<="'.$oFechaFin.'"';		
			}			
		}
...
DATE_FORMAT(ovv.OvvActaEntregaHora, "%H:%i") AS "NOvvActaEntregaHora",
		...
		ORDER BY pag.PagId ASC LIMIT 1
				...
				WHERE 1 = 1 '.$filtrar.$fecha.$tipo.$sucursal.$taentrega.$tcomprobante.$aprobacion1.$aprobacion2.$aprobacion3.$stipo.$estado.$moneda.$personal.$cliente.$cvehiculo.$ccotizacion.$vingreso.$estado.$facturable.$orden.$paginacion;
...
//		}
		...
		if(!empty($oEstado)){
			...
			foreach($oEstado as $elemento){
				 ...
				 if( !next( $oEstado ) ) {
					 ...
					 $estado .= '  (ovv.OvvEstado = "'.($elemento).'" ) OR';
				 }
		...
		if(!empty($oSucursal)){
			$sucursal = ' AND ovv.SucId = "'.$oSucursal.'"';
					...
					if(!empty($oVersion)){
			$vesion = ' AND ein.VveId = "'.$oVersion.'"';
		}
		
		
			if(!empty($oAnoFabricacion)){
			$efabricacion = ' AND ein.EinAnoFabricacion = "'.$oAnoFabricacion.'"';
		}
		if(!empty($oMes)){
		...
		if(!empty($oFuncion) & !empty($oParametro)){		
			...
			$sql = 'SELECT
				...
				WHERE 1 = 1 '.$ano.$mes.$filtrar.$sucursal.$fecha.$efabricacion.$vesion.$tipo.$stipo.$modelo.$marca.$estado.$moneda.$personal.$orden.$paginacion;
...
//									}
	...
	//Accion eliminar	 
	public function MtdActualizarAprobacion1OrdenVentaVehiculo($oElementos,$oEstado) {
		...
		$elementos = explode("#",$oElementos);
			...
			foreach($elementos as $elemento){
				...
				if(!empty($elemento)){
					...
					$sql = 'UPDATE tblovvordenventavehiculo SET OvvAprobacion1 = '.$oEstado.' WHERE OvvId = "'.$elemento.'"';
					$resultado = $this->InsMysql->MtdEjecutar($sql,false);        
			...
			if(!$error){			
				...
				if (!empty($this->OrdenVentaVehiculoObsequio)){		
					...
					foreach ($this->OrdenVentaVehiculoObsequio as $DatOrdenVentaVehiculoObsequio){
						...
						$InsOrdenVentaVehiculoObsequio->OvoObsequio = $DatOrdenVentaVehiculoObsequio->OvoObsequio;						
...
//			deb($this->OrdenVentaVehiculoMantenimiento);
	...
	public function MtdEditarOrdenVentaVehiculo() {
		...
		/*	if(empty($this->CliId)){
			...
			OvvTarjeta = "'.($this->OvvTarjeta).'",
			
			OvvReferencia = "'.($this->OvvReferencia).'",
			OvvTipoPlaca = "'.($this->OvvTipoPlaca).'",
			OvvGLPModeloTanque = "'.($this->OvvGLPModeloTanque).'",
...
//							$objPHPExcel->getActiveSheet()->getStyle('D4')->getFont()->setBold(true)->setSize(14);		
		...
		public function MtdEnviarCorreoSolicitarVINOrdenVentaVehiculo($oOrdenCompra,$oDestinatario,$oRemitente,$oAdjunto=array()){
		   ...
		   public function MtdObtenerOrdenVentaVehiculoPersonales($oCampo=NULL,$oCondicion="contiene",$oFiltro=NULL,$oOrden = 'OvvId',$oSentido = 'Desc',$oPaginacion = '0,10',$oFechaInicio=NULL,$oFechaFin=NULL,$oSucursal=NULL,$oVehiculoMarca=NULL) {
	...
	if(!empty($oVehiculoMarca)){
			...
			$sql = 'SELECT
				...
				LEFT JOIN tblvmavehiculomarca vma
				ON vmo.VmaId = vma.VmaId
				WHERE ovv.PerId IS NOT NULL
				 AND (
				 ovv.OvvEstado = 2 OR 
				 ovv.OvvEstado = 3 OR
				 ovv.OvvEstado = 4 OR
				 ovv.OvvEstado = 5
				 )   '.$filtrar.$fecha.$vmarca.$tipo.$dtransc.$sucursal.$stipo.$estado.$moneda.$personal.$ninteres.$orden.$paginacion;
            ...
            $InsOrdenVentaVehiculo = get_class($this);
				...
				while( $fila = $this->InsMysql->MtdObtenerDatos($resultado)){
					...
					$OrdenVentaVehiculo->PerApellidoPaterno = $fila['PerApellidoPaterno'];
					$OrdenVentaVehiculo->PerApellidoMaterno = $fila['PerApellidoMaterno'];
					$OrdenVentaVehiculo->PerNumeroDocumento = $fila['PerNumeroDocumento'];	
...
$InsAuditoria->AudUsuario = $oUsuario;
	...
	function FncNotificarFacturarOrdenVentaVehiculo($oOrdenVentaVehiculoId,$oTipoComprobante,$oUsuarioId,$oUsuario,$oDescripcionAdicional=NULL,$oPersonalNombre=NULL,$oPersonalFoto=NULL){
		...
		public function MtdEnviarBienvenidaOrdenVentaVehiculo($oOrdenVentaVehiculo,$oDestinatario,$oRemitente,$oAdjuntoBanner,$Imprimir=false){
			...
			global $SistemaCorreoRemitente;
			global $EmpresaMonedaId;
			global $FormularioNotaURLCitas;
			global $FormularioNotaCorreoRespuestaBienvenida;
			global $FormularioNotaCorreoNombreRespuestaBienvenida;
			...
			$mensaje .= "<span class='NombreCliente'>";
			$mensaje .= "<h4>";
			$mensaje .= "".$this->CliNombre." ".$this->CliApellidoPaterno." ".$this->CliApellidoMaterno;	
			$mensaje .= "</h4>";
			$mensaje .= "</span>";
			...
			$mensaje .= "<img width='600px' src = 'http://www.cisne.com.pe/correos/bienvenida/".$oAdjuntoBanner."'>";
			$mensaje .= "<br>";
			
			
			if(!empty(	$FormularioNotaURLCitas)){
				
				$mensaje  .= "<a href='".$FormularioNotaURLCitas."' target='_blank'>¡Agenda aqui tu mantenimiento!</a>";
			...
			$InsCorreo = new ClsCorreo();	
			//MtdEnviarCorreo($CorDestinatario,$CorRemitenteCorreo,$CorRemitenteNombre,$CorAsunto,$CorContenido,$oCorRutaAdjunto=NULL,$oCorAdjunto=NULL,$oResponder=NULL,$oResponderNombre=NULL)
			$InsCorreo->MtdEnviarCorreo($oDestinatario,$SistemaCorreoUsuario,$SistemaCorreoRemitente,"BIENVENID@ A LA FAMILIA CISNE",$mensaje,NULL,$oAdjunto,$FormularioNotaCorreoRespuestaBienvenida,$FormularioNotaCorreoNombreRespuestaBienvenida);
