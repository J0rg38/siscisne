...
//		}
		...
		if(!empty($oAno)){
			...
			$sql = 'SELECT
				...
				amo.AmoTipo = 2  
				AND amo.AmoSubTipo = 2
				'.$filtrar.$fecha.$tipo.$stipo.$estado.$faccion.$fingreso.$confactura.$conficha.$fiestado.$conboleta.$concrepuesto.$crestado.$pfacturar.$modalidad.$orden.$paginacion;
			...
			if(!$error){			
				...
				if (!empty($this->TallerPedidoDetalle)){		
					...
					foreach ($this->TallerPedidoDetalle as $DatTallerPedidoDetalle){
						...
						if($InsTallerPedidoDetalle->AmdEstado==3 and $InsTallerPedidoDetalle->AmdEliminado == 1 and ($InsTallerPedidoDetalle->AmdValidarStock == 1 and empty($InsTallerPedidoDetalle->AmdValidarStock))){
							...
							$StockReal = $InsAlmacenProducto->MtdObtenerAlmacenProductoStockActual($InsTallerPedidoDetalle->ProId,$InsTallerPedidoDetalle->AlmId,$Ano);
									...
									if(!empty($InsTallerPedidoDetalle->UmeId)){
										if(!empty($InsTallerPedidoDetalle->AmdCantidad)){
											if(!empty($InsTallerPedidoDetalle->AmdCantidadReal)){
												...
												//if(empty($InsTallerPedidoDetalle->FapId)){
//														
//														$InsFichaAccionProducto1 = new ClsFichaAccionProducto();
//														$InsFichaAccionProducto1->FapId = $DatTallerPedidoDetalle->FapId;
//														$InsFichaAccionProducto1->FccId = $this->FccId;
...
//deb($InsTallerPedidoDetalle->AmdFecha." ll");													$StockReal = 0;
...
//													$InsAlmacenProducto = new ClsAlmacenProducto();
//													
//													//deb($InsTallerPedidoDetalle->ProId." ".$InsTallerPedidoDetalle->AlmId." - ".$Ano);
//													
//													$StockReal = $InsAlmacenProducto->MtdObtenerAlmacenProductoStockActual($InsTallerPedidoDetalle->ProId,$InsTallerPedidoDetalle->AlmId,$Ano);
//													
//													if( ($StockReal + $InsTallerPedidoDetalle->AmdCantidadRealAnterior) < $InsTallerPedidoDetalle->AmdCantidadReal and $InsTallerPedidoDetalle->AmdEliminado == 1 and $InsTallerPedidoDetalle->AmdEstado == 3){
