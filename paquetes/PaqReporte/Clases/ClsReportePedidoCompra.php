...
/**
 * Description of ClsClsReportePedidoCompra
 *
 * @author Ing. Jonathan Blanco Alave
 */

class ClsReportePedidoCompra {
    ...
    public function MtdObtenerPedidoCompraDetalles($oCampo=NULL,$oFiltro=NULL,$oOrden = 'PcdId',$oSentido = 'Desc',$oPaginacion = '0,10',$oPedidoCompra=NULL,$oEstado=NULL,$oProducto=NULL,$oFechaInicio=NULL,$oFechaFin=NULL,$oOrdenCompra=NULL,$oCliente=NULL,$oConOrdenCompra=NULL,$oVentaDirectaDetalleId=NULL,$oPedidoCompraEstado=NULL,$oFecha="PcoFecha",$oValidarRecibido=false,$oConFichaIngreso=false,$oOrdenCompraEstado=NULL,$oSucursal=NULL) {
		...
		/*	(
			...
			*/
			...
			WHERE  1 = 1 '.$amovimiento.$estado.$producto.$filtrar.$fecha.$ocompra.$sucursal .$cocompra.$cliente.$vddetalle.$pcestado.$recibida.$fingreso.$ocestado.$orden.$paginacion;	
            ...
            $InsPedidoCompraDetalle = get_class($this);
				...
				while( $fila = $this->InsMysql->MtdObtenerDatos($resultado)){
					...
					$PedidoCompraDetalle->PcoTipoCambio = $fila['PcoTipoCambio'];
						...
						$PedidoCompraDetalle->MonNombre = $fila['MonNombre']; 
					$PedidoCompraDetalle->MonSimbolo = $fila['MonSimbolo'];
				...
				$PedidoCompraDetalle->SucNombre = (($fila['SucNombre']));
				
				
				$PedidoCompraDetalle->PerNombre = (($fila['PerNombre']));
				$PedidoCompraDetalle->PerApellidoPaterno = (($fila['PerApellidoPaterno']));
				$PedidoCompraDetalle->PerApellidoMaterno = (($fila['PerApellidoMaterno']));
