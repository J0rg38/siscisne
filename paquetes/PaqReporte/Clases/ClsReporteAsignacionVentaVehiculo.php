...
class ClsReporteAsignacionVentaVehiculo {
	...
	public function __destruct(){
	  ...
	  public function MtdObtenerAsignacionVentaVehiculoEntregas($oCampo=NULL,$oCondicion="contiene",$oFiltro=NULL,$oOrden = 'AvvId',$oSentido = 'Desc',$oPaginacion = '0,10',$oFechaInicio=NULL,$oFechaFin=NULL,$oEstado=NULL,$oOrdenVentaVehiculo=NULL,$oSucursal=NULL) {
...
//		}
				...
				if(!empty($oFechaInicio)){
					if(!empty($oFechaFin)){
						...
						DATE(
							IFNULL(ovv.OvvActaEntregaFecha,ovv.OvvActaEntregaFechaPDS)
						
						) >="'.$oFechaInicio.'" 
						AND DATE(IFNULL(ovv.OvvActaEntregaFecha,ovv.OvvActaEntregaFechaPDS)) 
						
						<="'.$oFechaFin.'"';
						
					}else{
						$fecha = ' AND  DATE(IFNULL(ovv.OvvActaEntregaFecha,ovv.OvvActaEntregaFechaPDS)) >="'.$oFechaInicio.'"';
					}
				}else{
					if(!empty($oFechaFin)){
						$fecha = ' AND DATE(IFNULL(ovv.OvvActaEntregaFecha,ovv.OvvActaEntregaFechaPDS))<="'.$oFechaFin.'"';		
					}			
				}
...
//		IFNULL(, (SELECT fin.FinFecha FROM tblfinfichaingreso fin WHERE fin.FinEstado <> 777 AND fin.FinTipo = 2 AND fin.EinId = avv.EinId ORDER BY fin.FinFecha DESC LIMIT 1)	)
		...
		if(($oConFechaEntrega)){
			...
			$entrega = ' AND ((ovv.OvvActaEntregaFecha IS NOT NULL AND ovv.OvvActaEntregaFecha != "0000-00-00" ) OR (ovv.OvvActaEntregaFechaPDS IS NOT NULL AND ovv.OvvActaEntregaFechaPDS != "0000-00-00")) ';
		}
	...
	if(!empty($oSucursal)){
		 ...
		 $sql = 'SELECT
			...
			ORDER BY pag.PagId ASC LIMIT 1
				...
				DATE_FORMAT(ovv.OvvActaEntregaFecha, "%d/%m/%Y") AS OvvActaEntregaFechaReal,
				ovv.OvvActaEntregaFecha,
				
				DATE_FORMAT(IFNULL(ovv.OvvActaEntregaFecha,ovv.OvvActaEntregaFechaPDS), "%d/%m/%Y") AS "NOvvActaEntregaFecha",
		...
		mon.MonSimbolo
            ...
            $InsAsignacionVentaVehiculo = get_class($this);
				...
				while( $fila = $this->InsMysql->MtdObtenerDatos($resultado)){
					...
					$AsignacionVentaVehiculo->OvvActaEntregaFecha = $fila['OvvActaEntregaFecha'];
					
					if(empty($AsignacionVentaVehiculo->OvvActaEntregaFecha ) ){
						$AsignacionVentaVehiculo->OvvActaEntregaFechaDirecto = 1;
					}else{
						$AsignacionVentaVehiculo->OvvActaEntregaFechaDirecto = 2;
					}
	  ...
	  public function MtdObtenerAsignacionVentaVehiculoNoEntregas($oCampo=NULL,$oCondicion="contiene",$oFiltro=NULL,$oOrden = 'AvvId',$oSentido = 'Desc',$oPaginacion = '0,10',$oFechaInicio=NULL,$oFechaFin=NULL,$oSucursal=NULL) {
		...
		if(!empty($oSucursal)){
		 ...
		 $sql = '
				...
				bol.BolFechaEmision
					...
					LIMIT 1
				),"0000-00-00") ), "%d/%m/%Y") AS "NRavComprobanteFecha",
				
				

DATEDIFF(DATE(NOW()), (IFNULL((
					SELECT 
					fac.FacFechaEmision
					FROM tblfacfactura fac
						LEFT JOIN tblftafacturatalonario fta
				...
				bol.BolFechaEmision
					...
					LIMIT 1
				),"0000-00-00") ) ) ) AS RavDiaTranscurrido,
				
				
				IFNULL(ovv.OvvActaEntregaFecha,ovv.OvvActaEntregaFechaPDS) AS RavFechaEntrega,
				...
				WHERE ovv.OvvActaEntregaFecha IS NULL 
				
				AND ovv.OvvActaEntregaFechaPDS IS NULL
				
				AND ovv.OvvEstado <> 6
				
				AND DATEDIFF(DATE(NOW()), (IFNULL((
					SELECT 
					fac.FacFechaEmision
					FROM tblfacfactura fac
						LEFT JOIN tblftafacturatalonario fta
				...
				bol.BolFechaEmision
				 ...
				 '.$filtrar.$fecha.$ovvehiculo.$sucursal.$tipo.$stipo.$entrega.$estado.$moneda.$cocompra.$vdirecta.$ocompra.$faccion.$fingreso.$orden.$paginacion;
            ...
            $InsAsignacionVentaVehiculo = get_class($this);
				...
				while( $fila = $this->InsMysql->MtdObtenerDatos($resultado)){
					...
					$AsignacionVentaVehiculo->OvvFecha = $fila['OvvFecha'];
					$AsignacionVentaVehiculo->RavComprobanteFecha = $fila['NRavComprobanteFecha'];
					$AsignacionVentaVehiculo->RavDiaTranscurrido = $fila['RavDiaTranscurrido'];					
					$AsignacionVentaVehiculo->RavFechaEntrega = $fila['RavFechaEntrega'];
					
					$AsignacionVentaVehiculo->PerNombreVendedor = $fila['PerNombre'];
					$AsignacionVentaVehiculo->PerApellidoPaternoVendedor = $fila['PerApellidoPaterno'];					
					$AsignacionVentaVehiculo->PerApellidoPaternoVendedor = $fila['PerApellidoPaterno'];
					
					$AsignacionVentaVehiculo->EinVIN = $fila['EinVIN'];
					$AsignacionVentaVehiculo->EinPlaca = $fila['EinPlaca'];					
					$AsignacionVentaVehiculo->EinAnoModelo = $fila['EinAnoModelo'];
					$AsignacionVentaVehiculo->EinAnoFabricacion = $fila['EinAnoFabricacion'];
					$AsignacionVentaVehiculo->EinColor = $fila['EinColor'];		
