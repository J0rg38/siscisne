...
//		}
				...
				if(!empty($oFechaInicio)){
					if(!empty($oFechaFin)){
						...
						$fecha = ' AND DATE(IFNULL(ovv.OvvActaEntregaFecha,IFNUL((SELECT fin.FinFecha FROM tblfinfichaingreso fin WHERE fin.FinEstado <> 777 AND fin.FinTipo = 2 AND fin.EinId = avv.EinId ORDER BY fin.FinFecha DESC LIMIT 1),"0000-00-00")))>="'.$oFechaInicio.'" AND DATE(IFNULL(ovv.OvvActaEntregaFecha,IFNUL((SELECT fin.FinFecha FROM tblfinfichaingreso fin WHERE fin.FinEstado <> 777 AND fin.FinTipo = 2 AND fin.EinId = avv.EinId ORDER BY fin.FinFecha DESC LIMIT 1),"0000-00-00"))) <="'.$oFechaFin.'"';
					}else{
						$fecha = ' AND  DATE(IFNULL(ovv.OvvActaEntregaFecha,IFNUL((SELECT fin.FinFecha FROM tblfinfichaingreso fin WHERE fin.FinEstado <> 777 AND fin.FinTipo = 2 AND fin.EinId = avv.EinId ORDER BY fin.FinFecha DESC LIMIT 1),"0000-00-00"))) >="'.$oFechaInicio.'"';
					}
				}else{
					if(!empty($oFechaFin)){
						$fecha = ' AND DATE(IFNULL(ovv.OvvActaEntregaFecha,IFNUL((SELECT fin.FinFecha FROM tblfinfichaingreso fin WHERE fin.FinEstado <> 777 AND fin.FinTipo = 2 AND fin.EinId = avv.EinId ORDER BY fin.FinFecha DESC LIMIT 1),"0000-00-00")))<="'.$oFechaFin.'"';		
					}			
				}
