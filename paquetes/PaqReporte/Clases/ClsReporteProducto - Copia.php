...
class ClsReporteProducto {
   ...
   public function MtdObtenerReporteProductoVentasPerdidas($oProductoId,$oFechaInicio=NULL,$oFechaFin=NULL,$oAno=NULL,$oMes=NULL,$oOrden=NULL,$oSentido=NULL,$oPaginacion=NULL,$oVehiculoMarca=NULL,$oSucursal=NULL){
	   ...
	   if(!empty($oProductoId)){
		...
		if(!empty($oMes)){
			...
			cpr.CprTipoCambio,
				...
				'.$producto.$fecha.$vmarca.$ano.$mes."  ".$orden."   ".$paginacion;
    ...
    public function MtdObtenerReporteProductoVentas($oProductoId,$oFechaInicio=NULL,$oFechaFin=NULL,$oAno=NULL,$oMes=NULL,$oOrden=NULL,$oSentido=NULL,$oPaginacion=NULL,$oVehiculoMarca=NULL,$oSucursal=NULL) {
...
if(!empty($oSucursal)){
		...
		$sql = 'SELECT
					...
					SUM(amd.AmdCantidad) AS RprCantidad,
					pro.ProABCInterno,
				...
				'.$producto.$fecha.$sucursal.$vmarca.$ano.$mes." GROUP BY amd.ProId HAVING RprCantidad>0 ".$orden."   ".$paginacion;
	 ...
	 public function MtdObtenerReporteProductoVentasPromedio($oProductoId,$oFecha=NULL,$oOrden=NULL,$oSentido=NULL,$oPaginacion=NULL,$oVehiculoMarca=NULL,$oDiasAtras=365,$oSucursal=NULL){
	   ...
	   if(!empty($oProductoId)){
		...
		if(!empty($oSucursal)){
			...
			$sql = 'SELECT
					...
					ROUND(((IFNULL(SUM(amd.AmdCantidad),0) / 365)),2) AS RprPromedioDiario,
					ROUND(((IFNULL(SUM(amd.AmdCantidad),0) / 12)),2) AS RprPromedioMensual,
					ROUND(((IFNULL(SUM(amd.AmdCantidad),0) / 4)),2) AS RprPromedioTrimestral,
					ROUND(((IFNULL(SUM(amd.AmdCantidad),0) / 2)),2) AS RprPromedioSemestral,
	...
	public function MtdObtenerReporteProductoStockMinimos($oCampo=NULL,$oCondicion=NULL,$oFiltro=NULL,$oOrden = 'ProId',$oSentido = 'Desc',$oPaginacion = '0,10',$oProductoTipo=NULL,$oProductoCategoria=NULL,$oProductoCritico=NULL,$oConStockMinimo=false) {
	...
	public function MtdObtenerReporteProductoValorInventarios($oCampo=NULL,$oCondicion=NULL,$oFiltro=NULL,$oOrden = 'ProId',$oSentido = 'Desc',$oPaginacion = '0,10',$oProductoTipo=NULL,$oProductoCategoria=NULL,$oSucursal=NULL,$oAlmacen=NULL,$oProductoId=NULL,$oAno) {
		...
		if(($oAno)){
			...
			$sql = 'SELECT
				...
				SUM(apr.AprStockReal) AS RpcStock,
				pro.ProCosto AS RpcCostoUnitario,
				SUM(apr.AprStockReal) * pro.ProCosto AS RpcValorInventario,
