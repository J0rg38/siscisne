...
//		}
				...
				if(!empty($oFechaInicio)){
					if(!empty($oFechaFin)){
						...
						DATE(
							IFNULL(ovv.OvvActaEntregaFecha,
								IFNULL(
									(
									SELECT 
									fin.FinFecha 
									FROM tblfinfichaingreso fin 
									WHERE fin.FinEstado <> 777 
									AND fin.FinTipo = 2 
									AND fin.EinId = avv.EinId 
						...
						AND DATE(IFNULL(ovv.OvvActaEntregaFecha,IFNULL((SELECT fin.FinFecha FROM tblfinfichaingreso fin WHERE fin.FinEstado <> 777 AND fin.FinTipo = 2 AND fin.EinId = avv.EinId ORDER BY fin.FinFecha DESC LIMIT 1),"0000-00-00"))) 
						
						<="'.$oFechaFin.'"';
						
					}else{
						$fecha = ' AND  DATE(IFNULL(ovv.OvvActaEntregaFecha,IFNULL((SELECT fin.FinFecha FROM tblfinfichaingreso fin WHERE fin.FinEstado <> 777 AND fin.FinTipo = 2 AND fin.EinId = avv.EinId ORDER BY fin.FinFecha DESC LIMIT 1),"0000-00-00"))) >="'.$oFechaInicio.'"';
					}
				}else{
					if(!empty($oFechaFin)){
						$fecha = ' AND DATE(IFNULL(ovv.OvvActaEntregaFecha,IFNULL((SELECT fin.FinFecha FROM tblfinfichaingreso fin WHERE fin.FinEstado <> 777 AND fin.FinTipo = 2 AND fin.EinId = avv.EinId ORDER BY fin.FinFecha DESC LIMIT 1),"0000-00-00")))<="'.$oFechaFin.'"';		
					}			
				}
		...
		$sql = 'SELECT
			...
			ORDER BY pag.PagId ASC LIMIT 1
				...
				ovv.OvvActaEntregaFecha,
				  ...
				  DATE_FORMAT(IFNULL(ovv.OvvActaEntregaFecha,IFNULL((SELECT fin.FinFecha FROM tblfinfichaingreso fin WHERE fin.FinEstado <> 777 AND fin.FinTipo = 2 AND fin.EinId = avv.EinId ORDER BY fin.FinFecha DESC LIMIT 1),"0000-00-00")), "%d/%m/%Y")
				  AS "NOvvActaEntregaFecha",
