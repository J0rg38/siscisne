...
//CLAVE SOL - FACTURADOR
$CLAVESOL_USUARIO = '20607780928MODDATOS';
$CLAVESOL_CONTRASENA = 'MODDATOS';

//CLAVE SOL - GUIA REMISION
...
//URL FACTURA
$URL_SERVICIO = 'https://demo-ose.nubefact.com/ol-ti-itcpe/billService';
$URL_SERVICIO_CDR = 'https://demo-ose.nubefact.com/ol-ti-itcpe/billService';
//$URL_SERVICIO_CDR_BETA = 'https://e-beta.sunat.gob.pe/ol-ti-itcpfegem/billConsultService';
...
function comprimirArchivoShell($archivoOrigen, $carpetaDestino)
{
    ...
    $archivoZip = rtrim($carpetaDestino, '/\\') . '/' . $nombreBase . '.zip';
...
function leerCDRXml($archivoZip, $carpeta = 'respuesta')
{
    ...
    if (!is_dir($rutaTemp)) {
        ...
        //rmdir($rutaTemp, true); // Limpiar la carpeta temporal si ya existe
    }
...
$xml = '20607780928-03-BB01-001210';

$cdr_leerCDRXml =  leerCDRXml("R-" . $xml . ".zip");

if ($cdr_leerCDRXml['ok']) {
    echo "document_id: " . $cdr_leerCDRXml['document_id'];
    echo "<br>";
    echo "response_code: " . $cdr_leerCDRXml['response_code'];
    echo "<br>";
    echo "description: " . $cdr_leerCDRXml['description'];
    echo "<br>";
    echo "response_date: " . $cdr_leerCDRXml['response_date'];
    echo "<br>";
    echo "response_time: " . $cdr_leerCDRXml['response_time'];
    echo "<br>";
} else {
    echo "Error al leer el CDR XML.";
    echo "<br>";
}

exit();

$resultado = firmarDocumentoConPEM($xml . ".xml");


if ($resultado['ok']) {

    $rutaFirmado = $resultado['archivo_firmado'];
    $zipNombre = $resultado['archivo_zip'];

    echo "rutaFirmado: " . $rutaFirmado;
    echo "<br>";
    echo "<br>";

    // $archivoXml = 'comprobantes/20607038431-03-B001-000312.xml';
    $rutaZip = comprimirArchivoShell($rutaFirmado, 'envio/');
    ...
    if ($rutaZip) {
        ...
        //var_dump($zipBytes);
        //echo "<br>";
        $resultadoProcesar = procesarComprobante($xml . ".zip", $zipBytes);
        //var_dump($resultadoProcesar);
        //echo "<br>";
        if ($resultadoProcesar['ok']) {


            $cdr_leerCDRXml =  leerCDRXml($resultadoProcesar['nombreArchivo']);
            ...
            echo "Procesado: " . $resultadoProcesar['mensaje'];
            echo "<br>";
            echo "archivo_cdr: " . $resultadoProcesar['archivo_cdr'];
            echo "<br>";
        } else {
            echo "Error: " . $resultadoProcesar['mensaje'];
            echo "<br>";
            echo "Detalle: " . $resultadoProcesar['response'];
            echo "<br>";
        }
    } else {
