...
function leerCDRXml($archivoZip, $carpeta = 'respuesta')
{
    ...
    global $CARPETA_RESPUESTA_GR;

    $carpeta = $CARPETA_RESPUESTA_GR;
...
function obtenerTokenSunat()
{
    ...
    curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30); // segundos para conectar
    curl_setopt($ch, CURLOPT_TIMEOUT, 60);        // segundos total ejecución
...
function MtdConsultarTicketAPI($oTicket, $oToken)
{
    ...
    curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 240); // tiempo máximo para conectar
    curl_setopt($ch, CURLOPT_TIMEOUT, 120);        // tiempo máximo total de la solicitud
                    ...
                    if ($resultado_MtdConsultarTicketAPI['codRespuesta'] == '0') {
                        ...
                        if ($resultado_MtdConsultarTicketAPI['arcCdr'] != '') {
                            ...
                            if ($resultado_leerCDRXml['ok']) {
                                ...
                                $Respuesta = "{'MensajeRespuesta':'" . $description . "','CodigoRespuesta':'" . $CodigoRespuesta . "','TicketRespuesta':'" . $document_id . "','FechaRespuesta':'" . $response_date . "','HoraRespuesta':'" . $response_time . "','Aux':'1','DigestValue':'','SignatureValue':'','XmlFirmado':'','ZIPRespuesta':'" . $contenidoBase64 . "'}";
                            } else {
                ...
                if (!$rutaZip) {
                    ...
                    if ($access_token) {
                        ...
                        if ($resultado_MtdEnviarDocumentoAPI['ok']) {
                            ...
                            $Respuesta = "{'MensajeRespuesta':'" . $resultado_MtdEnviarDocumentoAPI['mensaje'] . "','CodigoRespuesta':'" .  "P103" . "','TicketRespuesta':'" . "" . "','FechaRespuesta':'" . "" . "','HoraRespuesta':'" . "" . "','Aux':'1','DigestValue':'','SignatureValue':'','XmlFirmado':'','ZIPRespuesta':'" . "" . "'}";
                        }
                    } else {
                        $Respuesta = "{'MensajeRespuesta':'No se pudo obtener el token de acceso','CodigoRespuesta':'P107'}";
                    }
                }
            }
