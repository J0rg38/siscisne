...
$ResBoleta = $InsBoleta->MtdObtenerBoletas(NULL,NULL,NULL,"BolFechaEmision","DESC",NULL,NULL,FncCambiaFechaAMysql($FechaInicio),FncCambiaFechaAMysql($FechaFin),NULL,NULL,NULL,NULL,NULL,NULL);
...
if(!empty($ArrBoletas)){
	...
	foreach($ArrBoletas as $DatBoleta){
		...
		if($DatBoleta->MonId<>$EmpresaMonedaId){
				...
				$DatBoleta->BolSubTotal = ($DatBoleta->BolSubTotal / $DatBoleta->BolTipoCambio);
				$DatBoleta->BolImpuesto = ($DatBoleta->BolImpuesto  / $DatBoleta->BolTipoCambio);
				$DatBoleta->BolTotal = ($DatBoleta->BolTotal / $DatBoleta->BolTipoCambio);
				
		}
		
		echo "[Fila ".$fila."]> ";
		
		echo " ".$DatBoleta->BtaNumero." ".$DatBoleta->BolId;
		echo " ".$DatBoleta->CliNombre;
		
		
		$Comprobante['Numero'] = $DatBoleta->BolId;
		$Comprobante['Serie'] = $DatBoleta->BtaNumero;
		
		$Comprobante['Fecha'] = $DatBoleta->BolFechaEmision;
		$Comprobante['Cliente'] = $DatBoleta->CliNombre;
		$Comprobante['ClienteNumeroDocumento'] = $DatBoleta->CliNumeroDocumento;
		$Comprobante['ClienteDireccion'] = $DatBoleta->BolDireccion;
		$Comprobante['ClienteEmail'] = $DatBoleta->CliEmail;
		
		$Comprobante['Estado'] = $DatBoleta->BolEstado;
		$Comprobante['Clave'] = $DatBoleta->CliClaveElectronica;
		
		$Comprobante['MonedaId'] = $DatBoleta->MonId;
		
		$Comprobante['SubTotal'] = $DatBoleta->BolSubTotal;
		$Comprobante['Impuesto'] = $DatBoleta->BolImpuesto;
		$Comprobante['Total'] = $DatBoleta->BolTotal;
		$Comprobante['Tipo'] = 3;
		$Comprobante['TipoComprobante'] = 3;
		
		$Comprobante['ArchivoPDFNombre'] = $EmpresaCodigo.'-03-'.$DatBoleta->BtaNumero.'-'.$DatBoleta->BolId.".pdf";
		$Comprobante['ArchivoXMLNombre'] = $EmpresaCodigo.'-03-'.$DatBoleta->BtaNumero.'-'.$DatBoleta->BolId.".xml";
		$Comprobante['ArchivoCDRNombre'] = 'R-'.$EmpresaCodigo.'-03-'.$DatBoleta->BtaNumero.'-'.$DatBoleta->BolId.".zip";
			
		echo "<br>";
		echo "RutaArchivoPDF_alt: ";
		echo $RutaArchivoPDF_alt = "../generados/comprobantes_pdf/".str_replace(' ', '_', $EmpresaCodigo.'-03-'.$DatBoleta->BtaNumero.'-'.$DatBoleta->BolId.".pdf");
		echo "<br>";
		echo "<br>";		
		//echo "RutaArchivoXML: ";
		//echo $RutaArchivoXML = "../generados/comprobantes/".str_replace(' ', '_', $EmpresaCodigo.'-03-'.$DatBoleta->BtaNumero.'-'.$DatBoleta->BolId.".xml");
		//echo "<br>";
		echo "RutaArchivoXML_alt: ";
		echo $RutaArchivoXML_alt = "../generados/comprobantes_fir/".str_replace(' ', '_', $EmpresaCodigo.'-03-'.$DatBoleta->BtaNumero.'-'.$DatBoleta->BolId.".xml");
		echo "<br>";
		echo "RutaArchivoXML_alt2: ";
		echo $RutaArchivoXML_alt2 = "../recibidos/comprobantes/".str_replace(' ', '_', $EmpresaCodigo.'-03-'.$DatBoleta->BtaNumero.'-'.$DatBoleta->BolId.".xml");
		echo "<br>";
		echo "<br>";
			
		echo "RutaArchivoCDR: ";
		echo $RutaArchivoCDR_alt = "../generados/comprobantes_cdr/R-".str_replace(' ', '_', $EmpresaCodigo.'-03-'.$DatBoleta->BtaNumero.'-'.$DatBoleta->BolId.".zip");		
		echo "<br>";
		echo "RutaArchivoCDR_alt: ";
		echo $RutaArchivoCDR_alt2 = "../recibidos/comprobantes/R".str_replace(' ', '_', $EmpresaCodigo.'-03-'.$DatBoleta->BtaNumero.'-'.$DatBoleta->BolId.".zip");		
		echo "<br>";
		...
		//http://192.168.10.2:8080/sistema/recibidos/comprobantes/20410705878-03-B002-000296.xml
...
//		}
		...
		echo "<br>";	
			...
			$file = $EmpresaCodigo.'-03-'.$DatBoleta->BtaNumero.'-'.$DatBoleta->BolId;
			$remote_filePDF = '/comprobantes_pdf/'.$EmpresaCodigo.'-03-'.$DatBoleta->BtaNumero.'-'.$DatBoleta->BolId.".pdf";
			$remote_fileXML = '/comprobantes_xml/'.$EmpresaCodigo.'-03-'.$DatBoleta->BtaNumero.'-'.$DatBoleta->BolId.".xml";
			$remote_fileCDR = '/comprobantes_cdr/R-'.$EmpresaCodigo.'-03-'.$DatBoleta->BtaNumero.'-'.$DatBoleta->BolId.".zip";
...
//		echo "<br>";
	...
	echo "No se encontraron boletas<br />";
}
