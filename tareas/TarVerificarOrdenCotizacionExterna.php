...
$GET_GenerarExcel = (empty($_GET['GenerarExcel'])?1:$_GET['GenerarExcel']);
$GET_EnviarCorreo = (empty($_GET['GET_EnviarCorreo'])?2:$_GET['GET_EnviarCorreo']);

$GET_EnviarCorreo = 1;
$GET_GenerarExcel = 1;

//$Destinatarios = "jblanco@cyc.com.pe";
$Destinatarios = "cotizacionrepuestos.peru@gm.com,aliendo@cyc.com.pe,iquezada@cyc.com.pe,scanepam@cyc.com.pe,jblanco@cyc.com.pe,Pamela.Saldarriaga@Cevalogistics.com";
...
if(!empty($ArrOrdenCotizaciones)){
	foreach($ArrOrdenCotizaciones as $DatOrdenCotizacion){
		...
		$ResOrdenCotizacion = $InsOrdenCotizacion->MtdObtenerOrdenCotizaciones("OotCodigoExterno","esigual",$DatOrdenCotizacion->OotId,"OotId","DESC","1",NULL,NULL,NULL,NULL,0,NULL,NULL);
		$ArrOrdenCotizacions = $ResOrdenCotizacion['Datos'];
		
		if(empty($ArrOrdenCotizacions)){
			...
			if(!empty($DatOrdenCotizacion->ProCodigoOriginal)){
				...
				$InsOrdenCotizacion->OotCodigoExterno = $DatOrdenCotizacion->OotId;
				$InsOrdenCotizacion->OotOrigen = "OOT";
				$InsOrdenCotizacion->MonId = "MON-10001";
				$InsOrdenCotizacion->OotFecha = date("Y-m-d");
				...
				$InsOrdenCotizacion->OotIncluyeImpuesto = 2;	
				
				$InsOrdenCotizacion->ProPorcentajeDescuento = 0;
				$InsOrdenCotizacion->ProPorcentajeAdicional = 0;
				...
				$InsOrdenCotizacion->OotTotal = 0;
					...
					if(empty($ProductoId)){	
						...
						$InsProducto->ProTieneReemplazoGM = "";
						$InsProducto->ProTieneDisponibilidadGM = "";
				...
				if($InsOrdenCotizacion->MtdRegistrarOrdenCotizacion()){
					...
					if($GET_GenerarExcel==1){
						...
						if($InsOrdenCotizacion->MtdGenerarExcelOrdenCotizacion($InsOrdenCotizacion->OotId,"../")){
							...
							if($GET_EnviarCorreo==1){
								
	
								if($InsOrdenCotizacion->MtdEnviarCorreoPedidoOrdenCotizacion($InsOrdenCotizacion->OotId,$Destinatarios,"../")){
									...
									echo "Se envio correo electronico correctamente";
									echo "<br>";
								
								
								}else{
									
									echo "No se pudo enviar el correo electronico. PROCESO CANCELADO";
									echo "<br>";
								...
								echo "Enviar correo electronico DESACTIVADO";
								echo "<br>";
								
							}
							
						}else{
							echo "No se pudo generar el archivo excel. PROCESO CANCELADO";
							echo "<br>";
						}
			
					}else{
						
						echo "Generar archivo excel DESACTIVADO";
						echo "<br>";
