...
//MtdObtenerOrdenVentaVehiculoMantenimientos($oCampo=NULL,$oFiltro=NULL,$oOrden = 'OvmId',$oSentido = 'Desc',$oPaginacion = '0,10',$oOrdenVentaVehiculo=NULL,$oEstado=NULL) {
	...
	if(!empty($InsOrdenVentaVehiculo->OvvId)){
		...
		$InsOrdenVentaVehiculoMantenimiento = new ClsOrdenVentaVehiculoMantenimiento();													//	MAX(CONVERT(SUBSTR(AlmId,5),unsigned)) AS "MAXIMO"
		$ResOrdenVentaVehiculoMantenimiento = $InsOrdenVentaVehiculoMantenimiento->MtdObtenerOrdenVentaVehiculoMantenimientos(NULL,NULL,'CONVERT(OvmKilometraje,unsigned)','ASC',NULL,$InsOrdenVentaVehiculo->OvvId,1);
		$ArrOrdenVentaVehiculoMantenimientos = $ResOrdenVentaVehiculoMantenimiento['Datos'];	
		
		if(!empty($ArrOrdenVentaVehiculoMantenimientos)){
			
			foreach($ArrOrdenVentaVehiculoMantenimientos as $DatOrdenVentaVehiculoMantenimiento){
				
				if($DatOrdenVentaVehiculoMantenimiento->OvmRealizado=="No"){
